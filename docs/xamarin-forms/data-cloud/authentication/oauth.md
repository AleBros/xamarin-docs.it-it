---
title: Autenticazione degli utenti con un Provider di identità
...
![](oauth-images/google-auth.png "Uso AUTH per l'autenticazione con Google")
...
1. Andare alla [Console di Google API](http://console.developers.google.com) sito Web e accedere con le credenziali dell'account Google.
1. Dall'elenco a discesa progetto, selezionare un progetto esistente o crearne uno nuovo.
1. Nella barra laterale in "Gestione API", selezionare **credenziali**, quindi selezionare la **della scheda di schermata consenso OAuth**. Scegliere un' **indirizzo di posta elettronica**, specificare un **nome prodotto mostrato agli utenti**, quindi premere **Salva**.
...
![](oauth-images/login.png "Accesso di Google")
...
Il **Identifier** valore può essere qualsiasi oggetto e il **ruolo** valore deve essere impostato su **Visualizzatore**. Il **schemi Url** valore, che inizia con `com.googleusercontent.apps`, è possibile ottenere dall'id client iOS per il progetto [Console API Google](http://console.developers.google.com).
