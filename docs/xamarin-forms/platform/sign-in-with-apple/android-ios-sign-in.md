...
1. `applesignin_auth`: Genera l'URL di autorizzazione per l'accesso Apple e lo reindirizza.  Questa operazione viene eseguita sul lato server invece che sull'app per dispositivi mobili, quindi è possibile memorizzare `state` nella cache e convalidarla quando i server Apple inviano un callback.
...
L'app per dispositivi mobili si registra per gestire lo schema URI personalizzato selezionato (in questo caso `xamarinformsapplesignin://`), in `applesignin_callback` modo che la funzione possa inoltrare nuovamente i token.

Quando l'utente avvia l'autenticazione, si verificano i passaggi seguenti:

1. L'app per dispositivi mobili `nonce` genera `state` un valore e `applesignin_auth` e li passa alla funzione di Azure.
2. La `applesignin_auth` funzione di Azure genera un URL di autorizzazione di accesso Apple (usando `state` il `nonce`e fornito) e reindirizza il browser dell'app per dispositivi mobili.
...
5. La richiesta inviata da Apple alla `applesignin_callback` funzione viene convalidata per garantire che venga restituito il valore corretto `state` e che le attestazioni del token ID siano valide.
...
7. La `applesignin_callback` funzione di Azure reindirizza infine allo schema URI dell'app (`xamarinformsapplesignin://`) che aggiunge un frammento URI con i `xamarinformsapplesignin://#access_token=...&refresh_token=...&id_token=...`token (ad esempio).
8. L'app per dispositivi mobili analizza il frammento URI in `AppleAccount` un e convalida l' `nonce` attestazione ricevuta corrisponde `nonce` all'oggetto generato all'inizio del flusso.
...
- `APPLE_SIGNIN_SERVER_ID`: Si tratta dell' `ServerId` oggetto riportato in precedenza.  *Non* si tratta dell' _ID bundle_dell'app, ma piuttosto dell' *identificatore* dell' *ID Servizi* creato.
