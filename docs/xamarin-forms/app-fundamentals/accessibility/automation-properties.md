...
_Novell. Forms consente di impostare i valori di accessibilità negli elementi dell'interfaccia utente usando le proprietà associate dalla classe AutomationProperties, che a sua volta imposta i valori di accessibilità nativi. Questo articolo illustra come usare la classe AutomationProperties, in modo che un'utilità per la lettura dello schermo possa pronunciare gli elementi nella pagina._
...
> [!IMPORTANT]
> L'uso delle proprietà associate `AutomationProperties` può rallentare l'esecuzione del test dell'interfaccia utente in Android. Le [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) `AutomationProperties.Name` proprietà, e `AutomationProperties.HelpText` impostano entrambe la `ContentDescription` Proprietà nativa, con i `AutomationProperties.Name` valori delle proprietà e che `AutomationProperties.HelpText` hanno la precedenza sul `AutomationId` valore (se `AutomationProperties.Name` e `AutomationProperties.HelpText` sono impostati, i valori verranno concatenati). Ciò significa che i test che cercano `AutomationId` avranno esito negativo se anche `AutomationProperties.Name` o `AutomationProperties.HelpText` è impostato per l'elemento. In questo scenario, i test dell'interfaccia utente dovrebbero essere modificati per cercare il valore `AutomationProperties.Name` o `AutomationProperties.HelpText`, o una concatenazione dei due.
...
La proprietà associata `AutomationProperties.LabeledBy` consente a un altro elemento di definire le informazioni di accessibilità per l'elemento corrente. Ad esempio, un oggetto [`Label`](xref:Xamarin.Forms.Label) accanto a un oggetto [`Entry`](xref:Xamarin.Forms.Entry) può essere utilizzato per descrivere l'oggetto `Entry` rappresentato da. Per ottenere questo risultato, è possibile procedere come segue in XAML:
...
> [!NOTE]
> Si noti che [ `SetValue` ] (xrif: Xamarin.Forms . BindableObject. SetValue ( Xamarin.Forms . Il metodo BindableProperty, System. Object)) può essere usato anche per impostare la `AutomationProperties.IsInAccessibleTree` proprietà associata,`entry.SetValue(AutomationProperties.LabeledByProperty, nameLabel);`
...
In Android, per impostare il testo che l'utilità per la lettura dello schermo leggerà per la freccia indietro nella barra delle azioni di un [`NavigationPage`](xref:Xamarin.Forms.NavigationPage) , impostare le `AutomationProperties.Name` `AutomationProperties.HelpText` proprietà e in un oggetto [`Page`](xref:Xamarin.Forms.Page) . Tenere tuttavia presente che questo non ha nessun effetto sui pulsanti Indietro del sistema operativo.

### <a name="masterdetailpage"></a>MasterDetailPage

In iOS e nella piattaforma UWP (Universal Windows Platform) (UWP), per impostare il testo che lo screen reader leggerà per l'interruttore su un oggetto [`MasterDetailPage`](xref:Xamarin.Forms.MasterDetailPage) , impostare le `AutomationProperties.Name` proprietà, e `AutomationProperties.HelpText` su `MasterDetailPage` o sulla `IconImageSource` proprietà della `Master` pagina.

In Android, per impostare il testo che lo screen reader leggerà per l'interruttore in un [`MasterDetailPage`](xref:Xamarin.Forms.MasterDetailPage) , aggiungere risorse stringa al progetto Android:
...
In iOS, Android e UWP le utilità per la lettura dello schermo leggono il `Text` valore della proprietà delle [`ToolbarItem`](xref:Xamarin.Forms.ToolbarItem) istanze, purché `AutomationProperties.Name` `AutomationProperties.HelpText` i valori o non siano definiti.
