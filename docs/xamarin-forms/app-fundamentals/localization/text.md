...
Una volta creato un file di risorse predefinito, è possibile aggiungere testo creando `data` elementi all'interno dell'elemento `root` nel file di risorse:
...
L'impostazione dello **strumento personalizzato** su **PublicResXFileCodeGenerator** comporterà la generazione di una classe con accesso `public`. L'impostazione dello **strumento personalizzato** su **InternalResXFileCodeGenerator** comporterà la generazione di una classe generata con accesso `internal`. Un valore di **strumento personalizzato** vuoto non genererà una classe. Il nome della classe generata corrisponderà al nome del file di risorse. Il file **AppResources. resx** , ad esempio, comporterà la creazione di una classe `AppResources` in un file denominato **AppResources.designer.cs**.
...
Per il corretto funzionamento dei file di risorse, è necessario che nell'applicazione sia specificato un `NeutralResourcesLanguage`. Nel progetto condiviso, il file **AssemblyInfo.cs** deve essere personalizzato per specificare le impostazioni cultura predefinite. Il codice seguente illustra come impostare il `NeutralResourcesLanguage` su **en-US** nel file **AssemblyInfo.cs** :
...
> [!WARNING]
> Se non si specifica l'attributo `NeutralResourcesLanguage`, la classe `ResourceManager` restituisce `null` valori per tutte le impostazioni cultura senza un file di risorse specifico. Quando si specificano le impostazioni cultura predefinite, il `ResourceManager` restituisce i risultati dal file resx predefinito per le impostazioni cultura non supportate. È pertanto consigliabile specificare sempre il `NeutralResourcesLanguage` in modo che il testo venga visualizzato per le impostazioni cultura non supportate.
...
Le proprietà nella classe `AppResources` usano il valore corrente della `System.Globalization.CultureInfo.CurrentUICulture` per determinare il file di risorse delle impostazioni cultura da cui recuperare i valori.
...
Poiché ogni piattaforma archivia immagini con una struttura di file univoca, XAML usa la classe `OnPlatform` per impostare la proprietà `ImageSource` in base alla piattaforma corrente:
...
> [!NOTE]
> L'estensione di markup `OnPlatform` offre un modo più conciso per specificare valori specifici della piattaforma. Per altre informazioni, vedere [estensione di markup onplatform](~/xamarin-forms/xaml/markup-extensions/consuming.md#onplatform-markup-extension).

L'origine dell'immagine può essere impostata in base alla proprietà `Device.RuntimePlatform` nel codice:
...
- [Creare file di risorse per le app .NET](https://docs.microsoft.com/dotnet/framework/resources/creating-resource-files-for-desktop-apps)
