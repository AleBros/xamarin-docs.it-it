...
In un'applicazione Xamarin.Forms una vista è in genere [`Page`](xref:Xamarin.Forms.Page)una classe derivata [`ContentView`](xref:Xamarin.Forms.ContentView)da o derivata da. Tuttavia, le visualizzazioni possono essere rappresentate anche da un modello di dati, che specifica gli elementi dell'interfaccia utente da usare per rappresentare visivamente un oggetto quando viene visualizzato. Un modello di dati come vista non dispone di code-behind ed è progettato per l'associazione a un tipo di modello di visualizzazione specifico.
