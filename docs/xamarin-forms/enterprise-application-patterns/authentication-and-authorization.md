...
![](authentication-and-authorization-images/authentication.png "Autenticazione tramite un microservizio di autenticazione dedicato")
...
IdentityServer deve essere configurato nel `ConfigureServices` metodo nella `Startup` classe dell'applicazione Web chiamando il `services.AddIdentityServer` metodo, come illustrato nell'esempio di codice seguente dall'applicazione di riferimento eShopOnContainers:
...
Dopo la chiamata `services.AddIdentityServer` al metodo, vengono chiamate API Fluent aggiuntive per configurare gli elementi seguenti:
...
Quando si configurano le `AddInMemoryApiResources` risorse API, il `IEnumerable<ApiResource>` metodo prevede una raccolta. Nell'esempio di codice seguente viene `GetApis` illustrato il metodo che fornisce questa raccolta nell'applicazione di riferimento eShopOnContainers:
...
Questo metodo specifica che IdentityServer deve proteggere le API Orders e basket. Di conseguenza, i token di accesso gestiti di IdentityServer saranno necessari quando si effettuano chiamate a queste API. Per ulteriori informazioni sul `ApiResource` tipo, vedere la pagina relativa alle [risorse API](https://identityserver4.readthedocs.io/en/latest/reference/api_resource.html) nella documentazione di IdentityServer 4.

#### <a name="configuring-identity-resources"></a>Configurazione delle risorse di identità

Quando si configurano le `AddInMemoryIdentityResources` risorse Identity, il `IEnumerable<IdentityResource>` metodo prevede una raccolta. Le risorse di identità sono dati come l'ID utente, il nome o l'indirizzo di posta elettronica. Ogni risorsa di identità ha un nome univoco e possono essere assegnati tipi di attestazione arbitrari, che verranno quindi inclusi nel token di identità per l'utente. Nell'esempio di codice seguente viene `GetResources` illustrato il metodo che fornisce questa raccolta nell'applicazione di riferimento eShopOnContainers:
...
La specifica di OpenID Connect specifica alcune [risorse di identità standard](https://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims). Il requisito minimo è che è disponibile il supporto per la creazione di un ID univoco per gli utenti. Questa operazione viene eseguita esponendo `IdentityResources.OpenId` la risorsa di identità.

> [!NOTE]
> La `IdentityResources` classe supporta tutti gli ambiti definiti nella specifica di OpenID Connect (OpenID, posta elettronica, profilo, telefono e indirizzo).

IdentityServer supporta anche la definizione di risorse di identità personalizzate. Per ulteriori informazioni, vedere [definizione di risorse di identità personalizzate](http://docs.identityserver.io/en/latest/topics/resources.html#defining-custom-identity-resources) nella documentazione di IdentityServer. Per ulteriori informazioni sul `IdentityResource` tipo, vedere [risorsa Identity](https://identityserver4.readthedocs.io/en/latest/reference/identity_resource.html) nella documentazione di IdentityServer 4.
...
Quando si configurano `AddInMemoryClients` i client, il `IEnumerable<Client>` metodo prevede una raccolta. L'esempio di codice seguente illustra la configurazione per l'app per dispositivi mobili `GetClients` eShopOnContainers nel metodo che fornisce questa raccolta nell'applicazione di riferimento eShopOnContainers:
...
- `ClientName`: Nome visualizzato del client utilizzato per la registrazione e la schermata di consenso.
...
Il flusso di autenticazione tra un client e IdentityServer può essere configurato specificando i tipi di concessione `Client.AllowedGrantTypes` nella proprietà. Le specifiche di OpenID Connect e OAuth 2,0 definiscono una serie di flussi di autenticazione, tra cui:

- Implicita. Questo flusso è ottimizzato per le applicazioni basate su browser e deve essere usato per le richieste di autenticazione utente o per l'autenticazione e il token di accesso. Tutti i token vengono trasmessi tramite il browser e pertanto le funzionalità avanzate come i token di aggiornamento non sono consentite.
...
Viene eseguita una richiesta di accesso a `<base endpoint>:5105/connect/authorize`. Dopo l'autenticazione riuscita, IdentityServer restituisce una risposta di autenticazione contenente un codice di autorizzazione e un token di identità. Il codice di autorizzazione viene quindi inviato `<base endpoint>:5105/connect/token`a, che risponde con i token di accesso, identità e aggiornamento.

L'app per dispositivi mobili eShopOnContainers si disconnette da IdentityServer inviando una richiesta `<base endpoint>:5105/connect/endsession`a, con parametri aggiuntivi. Dopo la disconnessione, IdentityServer risponde inviando un URI di reindirizzamento dopo la disconnessione all'app per dispositivi mobili. La figura 9-3 illustra questo processo.
...
Nell'app per dispositivi mobili eShopOnContainers, la comunicazione con IdentityServer viene eseguita `IdentityService` dalla classe, che implementa `IIdentityService` l'interfaccia. Questa interfaccia specifica che la classe di implementazione deve `CreateAuthorizationRequest`fornire `CreateLogoutRequest`i metodi `GetTokenAsync` , e.

#### <a name="signing-in"></a>Accesso

Quando l'utente tocca il pulsante di **accesso** in `LoginView` `LoginViewModel` , viene `SignInCommand` eseguito il nella classe, che a sua volta esegue il `SignInAsync` metodo. L'esempio di codice seguente illustra il metodo:
...
Questo metodo richiama il `CreateAuthorizationRequest` metodo `IdentityService` nella classe, illustrato nell'esempio di codice seguente:
...
Questo metodo crea l'URI per l' [endpoint di autorizzazione](https://identityserver4.readthedocs.io/en/latest/endpoints/authorize.html)di IdentityServer, con i parametri obbligatori. L'endpoint di autorizzazione si `/connect/authorize` trova sulla porta 5105 dell'endpoint di base esposto come impostazione utente. Per ulteriori informazioni sulle impostazioni utente, vedere [Configuration Management](~/xamarin-forms/enterprise-application-patterns/configuration-management.md).
...
L'URI restituito viene archiviato nella `LoginUrl` proprietà `LoginViewModel` della classe. Quando la `IsLogin` proprietà diventa `true`, il [`WebView`](xref:Xamarin.Forms.WebView) in `LoginView` diventa visibile. I `WebView` dati associano [`Source`](xref:Xamarin.Forms.WebView.Source) la relativa proprietà `LoginUrl` `LoginViewModel` alla proprietà della classe, quindi effettua una richiesta di accesso a IdentityServer quando la proprietà è `LoginUrl` impostata sull'endpoint di autorizzazione di IdentityServer. Quando IdentityServer riceve la richiesta e l'utente non `WebView` viene autenticato, verrà reindirizzato alla pagina di accesso configurata, come illustrato nella figura 9-4.

![](authentication-and-authorization-images/login.png "Pagina di accesso visualizzata da WebView")

**Figura 9-4:** Pagina di accesso visualizzata da WebView

Una volta completato l' [`WebView`](xref:Xamarin.Forms.WebView) accesso, verrà reindirizzato a un URI restituito. Questa `WebView` navigazione determinerà `NavigateAsync` l'esecuzione del `LoginViewModel` metodo nella classe, come illustrato nell'esempio di codice seguente:
...
Questo metodo analizza la risposta di autenticazione contenuta nell'URI restituito e, a condizione che sia presente un codice di autorizzazione valido, esegue una richiesta all' [endpoint token](https://identityserver4.readthedocs.io/en/latest/endpoints/token.html)di IdentityServer, passando il codice di autorizzazione, PKCE Secret Verifier, e altri parametri obbligatori. L'endpoint del token si `/connect/token` trova sulla porta 5105 dell'endpoint di base esposto come impostazione utente. Per ulteriori informazioni sulle impostazioni utente, vedere [Configuration Management](~/xamarin-forms/enterprise-application-patterns/configuration-management.md).
...
Se l'endpoint del token riceve un codice di autorizzazione valido e PKCE Secret Verifier, risponde con un token di accesso, un token di identità e un token di aggiornamento. Il token di accesso, che consente l'accesso alle risorse API, e il token di identità vengono quindi archiviati come impostazioni dell'applicazione e l'esplorazione delle pagine viene eseguita. Di conseguenza, l'effetto complessivo nell'app per dispositivi mobili eShopOnContainers è il seguente: purché gli utenti siano in grado di eseguire l'autenticazione con IdentityServer, vengono spostati `MainView` nella pagina, ovvero un [`TabbedPage`](xref:Xamarin.Forms.TabbedPage) che Visualizza il `CatalogView` come scheda selezionata.
...
> [!NOTE]
> EShopOnContainers consente anche l'accesso fittizio quando l'app è configurata per l'uso di servizi fittizi `SettingsView`in. In questa modalità l'app non comunica con IdentityServer, ma consente all'utente di eseguire l'accesso con qualsiasi credenziale.

#### <a name="signing-out"></a>Disconnessione

Quando l'utente tocca il **pulsante Disconnetti** `ProfileView`in `ProfileViewModel` , viene eseguito l' `LogoutCommand` oggetto nella classe, che a sua volta esegue il `LogoutAsync` metodo. Questo metodo esegue l'esplorazione della pagina `LoginView` sulla pagina, passando `LogoutParameter` un'istanza impostata `true` su come parametro. Per ulteriori informazioni sul passaggio di parametri durante l'esplorazione delle pagine, vedere [passaggio di parametri durante la navigazione](~/xamarin-forms/enterprise-application-patterns/navigation.md#passing_parameters_during_navigation).

Quando si crea e si passa a una vista, viene `InitializeAsync` eseguito il metodo del modello di visualizzazione associato della visualizzazione, che esegue quindi il `Logout` metodo della `LoginViewModel` classe, come illustrato nell'esempio di codice seguente:
...
Questo metodo richiama il `CreateLogoutRequest` metodo `IdentityService` nella classe, passando il token di identità recuperato dalle impostazioni dell'applicazione come parametro. Per ulteriori informazioni sulle impostazioni dell'applicazione, vedere [Configuration Management](~/xamarin-forms/enterprise-application-patterns/configuration-management.md). L'esempio di codice seguente illustra il metodo `CreateLogoutRequest`:
...
Questo metodo crea l'URI dell'endpoint della [sessione finale](https://identityserver4.readthedocs.io/en/latest/endpoints/endsession.html#refendsession)di IdentityServer con i parametri necessari. L'endpoint della sessione finale si `/connect/endsession` trova sulla porta 5105 dell'endpoint di base esposto come impostazione utente. Per ulteriori informazioni sulle impostazioni utente, vedere [Configuration Management](~/xamarin-forms/enterprise-application-patterns/configuration-management.md).

L'URI restituito viene archiviato nella `LoginUrl` proprietà `LoginViewModel` della classe. Mentre la `IsLogin` proprietà è `true`, l' [`WebView`](xref:Xamarin.Forms.WebView) oggetto in `LoginView` è visibile. I `WebView` dati associano [`Source`](xref:Xamarin.Forms.WebView.Source) la relativa proprietà `LoginUrl` `LoginViewModel` alla proprietà della classe, quindi effettua una richiesta di disconnessione a IdentityServer quando la proprietà `LoginUrl` è impostata sull'endpoint della sessione finale di IdentityServer. Quando IdentityServer riceve questa richiesta, a condizione che l'utente abbia eseguito l'accesso, si verifica la disconnessione. L'autenticazione viene rilevata con un cookie gestito dal middleware di autenticazione dei cookie da ASP.NET Core. Quindi, la disconnessione da IdentityServer rimuove il cookie di autenticazione e invia di nuovo un URI di reindirizzamento post-disconnessione al client.

Nell'app per dispositivi mobili, [`WebView`](xref:Xamarin.Forms.WebView) verrà reindirizzato all'URI di reindirizzamento post-disconnessione. Questa `WebView` navigazione determinerà `NavigateAsync` l'esecuzione del `LoginViewModel` metodo nella classe, come illustrato nell'esempio di codice seguente:
...
Questo metodo cancella sia il token di identità che il token di accesso dalle impostazioni dell'applicazione e imposta `IsLogin` la `false`proprietà su, `LoginView` che fa [`WebView`](xref:Xamarin.Forms.WebView) sì che l'oggetto nella pagina diventi invisibile. Infine, la `LoginUrl` proprietà viene impostata sull'URI dell' [endpoint di autorizzazione](https://identityserver4.readthedocs.io/en/latest/endpoints/authorize.html)di IdentityServer, con i parametri obbligatori, in preparazione per la prossima volta che l'utente avvia un accesso.
...
Se un utente non autorizzato tenta di accedere a un controller o a un'azione contrassegnata con `Authorize` l'attributo, il framework MVC restituisce un codice di stato HTTP 401 (non autorizzato).

> [!NOTE]
> I `Authorize` parametri possono essere specificati nell'attributo per limitare un'API a utenti specifici. Per ulteriori informazioni, vedere [autorizzazione](/aspnet/core/security/authorization/introduction/).
...
![](authentication-and-authorization-images/authorization.png "Autorizzazione per token di accesso")
...
Per eseguire l'autorizzazione con IdentityServer, è necessario aggiungere il middleware di autorizzazione alla pipeline di richieste HTTP dell'applicazione Web. Il middleware viene aggiunto nel `ConfigureAuth` metodo nella `Startup` classe dell'applicazione Web, `Configure` che viene richiamato dal metodo e viene illustrato nell'esempio di codice seguente dall'applicazione di riferimento eShopOnContainers:
...
> [!NOTE]
> Il middleware di autorizzazione di IdentityServer deve essere aggiunto alla pipeline di richieste HTTP dell'applicazione Web prima di `app.UseMvc()` aggiungere `app.UseMvcWithDefaultRoute()`MVC con o.
...
Il token di accesso viene archiviato come impostazione dell'applicazione e viene recuperato dall'archiviazione specifica della piattaforma e incluso nella chiamata al `GetOrderAsync` metodo `OrderService` nella classe.
...
Il token di accesso viene recuperato dall'archiviazione specifica della piattaforma e incluso nella chiamata al `UpdateBasketAsync` metodo `BasketService` nella classe.

La `RequestProvider` classe, nell'app per dispositivi mobili eShopOnContainers, USA `HttpClient` la classe per eseguire richieste alle API RESTful esposte dall'applicazione di riferimento eShopOnContainers. Quando si effettuano richieste alle API Order e basket, che richiedono l'autorizzazione, è necessario includere nella richiesta un token di accesso valido. Questa operazione viene eseguita aggiungendo il token di accesso alle intestazioni dell' `HttpClient` istanza, come illustrato nell'esempio di codice seguente:

```csharp
httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
```

La `DefaultRequestHeaders` proprietà `Authorization` `Bearer`della classe espone le intestazioni inviate con ogni richiesta e il token di accesso viene aggiunto all'intestazione preceduta dalla stringa. `HttpClient` Quando la richiesta viene inviata a un'API RESTful, il valore dell' `Authorization` intestazione viene estratto e convalidato per assicurarsi che venga inviato da un'autorità emittente attendibile e usato per determinare se l'utente dispone dell'autorizzazione per richiamare l'API che lo riceve.
