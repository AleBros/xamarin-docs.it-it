...
Le risorse vengono definite un [ `ResourceDictionary` ](https://developer.xamarin.com/api/type/Xamarin.Forms.ResourceDictionary/) vale a dire quindi impostato su uno dei seguenti `Resources` proprietà:

- Il [ `Resources` ](https://developer.xamarin.com/api/property/Xamarin.Forms.Application.Resources/) proprietà di qualsiasi classe che deriva da [`Application`](https://developer.xamarin.com/api/type/Xamarin.Forms.Application/)
- Il [ `Resources` ](https://developer.xamarin.com/api/property/Xamarin.Forms.VisualElement.Resources/) proprietà di qualsiasi classe che deriva da ['VisualElement'](https://developer.xamarin.com/api/type/Xamarin.Forms.Application/)
...
Ciò `ResourceDictionary` definisce tre [ `Color` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Color/) risorse e un [ `Style` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Style/) risorse. Per ulteriori informazioni sul `App` classe, vedere [classe App](~/xamarin-forms/app-fundamentals/application-class.md).
...
Ogni risorsa ha una chiave specificata utilizzando il `x:Key` attributo, che diventa il dizionario chiave nella `ResourceDictionary`. La chiave viene usata per recuperare una risorsa dal `ResourceDictionary` dal [ `StaticResource` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Xaml.StaticResourceExtension/) estensione di markup, come illustrato nell'esempio di codice XAML seguente che mostra le risorse aggiuntive definite all'interno di `StackLayout`:
...
Si noti tuttavia che la barra in background del [ `NavigationPage` ](https://developer.xamarin.com/api/type/Xamarin.Forms.NavigationPage/) è ancora giallo, perché il [ `BarBackgroundColor` ](https://developer.xamarin.com/api/property/Xamarin.Forms.NavigationPage.BarBackgroundColor/) proprietà è impostata sul valore della `PageBackgroundColor` risorsa definita nell'applicazione livello `ResourceDictionary`.
...
Nell'esempio XAML seguente viene illustrata una [ `ResourceDictionary` ](https://developer.xamarin.com/api/type/Xamarin.Forms.ResourceDictionary/) denominato `MyResourceDictionary`:
...
I dizionari risorse sottoposti a merge combinano uno o più `ResourceDictionary` le istanze in un altro `ResourceDictionary`. È possibile farlo in un file XAML impostando i [ `MergedDictionaries` ](https://developer.xamarin.com/api/property/Xamarin.Forms.ResourceDictionary.MergedDictionaries/) proprietà da uno o più dizionari risorse che vengono unite all'applicazione, una pagina o un livello di controllo `ResourceDictionary`.

> [!IMPORTANT]
> `ResourceDictionary` definisce inoltre un [ `MergedWith` ](https://developer.xamarin.com/api/property/Xamarin.Forms.ResourceDictionary.MergedWith/) proprietà. Non utilizzare questa proprietà. ed è stato deprecato a partire da 3.0 xamarin. Forms.
...
1. Le risorse contenute nel dizionario risorse che è stato unito tramite deprecate [ `MergedWith` ](https://developer.xamarin.com/api/property/Xamarin.Forms.ResourceDictionary.MergedWith/) proprietà.
...
A partire da xamarin. Forms 3.0, il processo di unione `ResourceDictionaries` è diventata più semplice e più flessibile. Il `MergedDictionaries` i tag di elemento di proprietà non sono più necessari. Al contrario, aggiungere al dizionario risorse un'altra `ResourceDictionary` tag con il nuovo [ `Source` ](https://developer.xamarin.com/api/property/Xamarin.Forms.ResourceDictionary.Source/) impostata per il nome del file del file XAML con le risorse:
