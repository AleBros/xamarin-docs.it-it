...
description: Ricarica immediatamente le modifiche apportate al file XAML nell'applicazione in esecuzione, in modo da non dover compilare il progetto Novell. Forms dopo ogni modifica del codice XAML.
ms.prod: xamarin
...
## <a name="enable-xaml-hot-reload-for-xamarinforms"></a>Abilitare il ricaricamento a caldo di XAML per Novell. Forms
...
Se si sta lavorando da una soluzione Novell. Forms esistente, non è necessaria alcuna installazione aggiuntiva per usare il ricaricamento a caldo di XAML, ma potrebbe essere necessario controllare la configurazione per garantire la migliore esperienza. Per prima cosa, abilitarla nelle impostazioni dell'IDE:

* In Windows selezionare la casella di controllo **Abilita ricarica a caldo Novell** in **strumenti** > **Opzioni** > **Novell** > **ricarica a caldo**.
* In Mac, selezionare la casella di controllo **Abilita ricaricamento a caldo di Novell** in **Visual Studio** > **preferenze** > **strumenti per Novell** > il **ricaricamento a caldo di XAML**.
  * Nelle versioni precedenti di Visual Studio per Mac il menu è in **Visual Studio** > **preferenze** > **progetti** > **ricaricamento a caldo Novell**.
...
Per segnalare un bug, usare la **guida** > **inviare commenti e suggerimenti** > **segnalare un problema** in Windows e **aiutare** > **segnalare un problema** in Mac.
