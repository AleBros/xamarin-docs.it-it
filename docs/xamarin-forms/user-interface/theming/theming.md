...
> [!IMPORTANT]
> Usare l' `StaticResource` estensione di markup se non è necessario modificare il tema dell'app in fase di esecuzione.
...
Un tema è definito come una raccolta di oggetti risorsa archiviati in un oggetto [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary) .

Nell'esempio seguente viene illustrata l' `LightTheme` applicazione di esempio:
...
Nell'esempio seguente viene illustrata l' `DarkTheme` applicazione di esempio:
...
> [!IMPORTANT]
> Per ogni oggetto `ResourceDictionary` , che chiama il metodo, è necessario un file code-behind `InitializeComponent` . Questa operazione è necessaria in modo che un oggetto CLR che rappresenta il tema scelto possa essere creato in fase di esecuzione.

## <a name="set-a-default-theme"></a>Imposta un tema predefinito

Un'applicazione richiede un tema predefinito, in modo che i controlli dispongano di valori per le risorse che utilizzano. È possibile impostare un tema predefinito unendo il tema nell'oggetto a [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary) livello di applicazione `ResourceDictionary` definito in **app. XAML**:
...
Quando un'applicazione vuole usare una risorsa archiviata in un oggetto [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary) che rappresenta un tema, questa operazione deve essere eseguita con l' `DynamicResource` estensione di markup. In questo modo si garantisce che se si seleziona un tema diverso in fase di esecuzione, verranno applicati i valori del nuovo tema.

Nell'esempio seguente vengono illustrati tre stili dell'applicazione di esempio che possono essere applicati agli [`Label`](xref:Xamarin.Forms.Label) oggetti:
...
Questi stili sono definiti nel dizionario risorse a livello di applicazione, in modo che possano essere utilizzati da più pagine. Ogni stile utilizza le risorse del tema con l' `DynamicResource` estensione di markup.
...
1. Rimuovere il tema corrente dall'applicazione. Questa operazione viene eseguita deselezionando la [`MergedDictionaries`](xref:Xamarin.Forms.ResourceDictionary.MergedDictionaries) proprietà a livello di applicazione [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary) .
2. Carica il tema selezionato. Questa operazione viene eseguita aggiungendo un'istanza del tema selezionato alla proprietà a `MergedDictionaries` livello di applicazione `ResourceDictionary` .

Tutti [`VisualElement`](xref:Xamarin.Forms.VisualElement) gli oggetti che impostano le proprietà con l' `DynamicResource` estensione di markup applicheranno quindi i nuovi valori del tema. Questo problema si verifica perché l' `DynamicResource` estensione di markup mantiene un collegamento alle chiavi del dizionario. Pertanto, quando i valori associati alle chiavi vengono sostituiti, le modifiche vengono applicate agli `VisualElement` oggetti.

Nell'applicazione di esempio, viene selezionato un tema tramite una pagina modale che contiene un oggetto [`Picker`](xref:Xamarin.Forms.Picker) . Il codice seguente illustra il `OnPickerSelectionChanged` metodo, che viene eseguito quando il tema selezionato cambia:
