...
Il [MonkeyAppCSS](https://developer.xamarin.com/samples/xamarin-forms/UserInterface/Styles/MonkeyAppCSS/) illustra l'uso di CSS per applicare uno stile a una semplice app di esempio e viene illustrato negli screenshot seguenti:
...
Il primo argomento per il `StyleSheet.FromAssemblyResource` metodo è l'assembly contenente il foglio di stile, mentre il secondo argomento è un `string` che rappresenta l'identificatore della risorsa. L'identificatore della risorsa può essere ottenuto dal **proprietà** finestra quando viene selezionato il file CSS.

In alternativa, è possibile caricare un foglio di stile da un `StringReader` e aggiunto a un [ `ResourceDictionary` ](xref:Xamarin.Forms.ResourceDictionary):
...
|`border-radius`|`BoxView`|_Valore Double_ \| `initial` |`border-radius: 10;`|
|`border-width`|`Button`|_Valore Double_ \| `initial` |`border-width: .5;`|
|`color`|`ActivityIndicator`, `BoxView`, `Button`, `DatePicker`, `Editor`, `Entry`, `Label`, `Picker`, `ProgressBar`, `SearchBar`, `Switch`, `TimePicker`|_Colore_ \| `initial` |`color: rgba(255, 0, 0, 0.3);`|
...
|`padding-left`|`Layout`, `Page`|_Valore Double_ \| `initial`|`padding-left: 3;`|
|`padding-top`|`Layout`, `Page`| _Valore Double_ \| `initial` |`padding-top: 4;`|
|`padding-right`|`Layout`, `Page`| _Valore Double_ \| `initial` |`padding-right: 2;`|
|`padding-bottom`|`Layout`, `Page`| _Valore Double_ \| `initial` |`padding-bottom: 6;`|
...
**Xamarin. Forms 3.0 CSS, da [Xamarin University](https://university.xamarin.com/)**
