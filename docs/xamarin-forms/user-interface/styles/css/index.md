...
> [!NOTE]
> Il selettore `element element` non richiede che l'elemento figlio sia un figlio _diretto_ del padre. l'elemento figlio potrebbe avere un padre diverso. Si verifica una selezione purché un predecessore sia il primo elemento specificato.
...
|---|---|---|
|`.class`|`.header`|Seleziona tutti gli elementi con la proprietà `StyleClass` contenente ' header '. Si noti che questo selettore distingue tra maiuscole e minuscole|
|`#id`|`#email`|Seleziona tutti gli elementi con `StyleId` impostato su `email`. Se `StyleId` non è impostato, eseguire il fallback su `x:Name`. Quando si usa XAML, è preferibile `x:Name` `StyleId`. Si noti che questo selettore distingue tra maiuscole e minuscole|
...
|`^base`|`^contentpage`|Seleziona tutti gli elementi con `ContentPage` come classe di base, incluso `ContentPage` stesso. Si noti che questo selettore non fa distinzione tra maiuscole e minuscole e non fa parte della specifica CSS.|
|`element,element`|`label,button`|Seleziona tutti gli elementi di `Button` e tutti gli elementi di `Label`. Si noti che questo selettore non distingue tra maiuscole e minuscole.|
|`element element`|`stacklayout label`|Seleziona tutti gli elementi di `Label` all'interno di un `StackLayout`. Si noti che questo selettore non distingue tra maiuscole e minuscole.|
|`element>element`|`stacklayout>label`|Seleziona tutti gli elementi di `Label` con `StackLayout` come elemento padre diretto. Si noti che questo selettore non distingue tra maiuscole e minuscole.|
|`element+element`|`label+entry`|Seleziona tutti gli elementi di `Entry` direttamente dopo una `Label`. Si noti che questo selettore non distingue tra maiuscole e minuscole.|
|`element~element`|`label~entry`|Seleziona tutti gli elementi di `Entry` preceduti da un `Label`. Si noti che questo selettore non distingue tra maiuscole e minuscole.|
...
|`text-align`| `Entry`, `EntryCell`, `Label`, `SearchBar`|`left` \| `top` \| `right` \| `bottom` \| `start` \| 0 1 2 3 4 5 6. `left` e `right` devono essere evitati negli ambienti da destra a sinistra.| `text-align: right;`|
