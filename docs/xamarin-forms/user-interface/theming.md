...
Nell'esempio seguente viene illustrata `LightTheme` l'applicazione di esempio:
...
Nell'esempio seguente viene illustrata `DarkTheme` l'applicazione di esempio:
...
Ogni [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary) `ResourceDictionary` contiene [`Color`](xref:Xamarin.Forms.Color) risorse che definiscono i rispettivi temi, ognuno dei quali usa valori di chiave identici. Per altre informazioni sui dizionari risorse, vedere [dizionari risorse](~/xamarin-forms/xaml/resource-dictionaries.md).
...
Per ulteriori informazioni sull'Unione dei dizionari risorse, vedere [Unione di dizionari in Novell. forms 3,0](~/xamarin-forms/xaml/resource-dictionaries.md#merging-dictionaries-in-xamarinforms-30).
