...
Come si è visto nel [tradurre la trasformazione](~/xamarin-forms/user-interface/graphics/skiasharp/transforms/translate.md) articolo, la trasformazione di traslazione può spostare un oggetto grafico da una posizione a un'altra. Al contrario, la trasformazione di scala modifica la dimensione dell'oggetto grafico:
...
Il `SKCanvas` classe definisce quattro `Scale` metodi. Il primo [ `Scale` ](https://developer.xamarin.com/api/member/SkiaSharp.SKCanvas.Scale/p/System.Single/) metodo è per scomporre nei casi in cui si desidera che la stessa scalabilità orizzontale e verticale:
...
La seconda [ `Scale` ](https://developer.xamarin.com/api/member/SkiaSharp.SKCanvas.Scale/p/System.Single/System.Single/) metodo consente di specificare valori diversi per la scalabilità orizzontale e verticale:
...
La terza [ `Scale` ](https://developer.xamarin.com/api/member/SkiaSharp.SKCanvas.Scale/p/SkiaSharp.SKPoint/) metodo combina i due fattori di scala in un singolo `SKPoint` valore:
...
Il ridimensionamento è rispetto all'angolo superiore sinistro dell'area di disegno. Ciò potrebbe essere esattamente ciò che si desidera, ma potrebbe non essere. Si supponga che si desidera posizionare il testo e il rettangolo in un'altra posizione nell'area di disegno e si desidera che per la scalabilità rispetto al relativo centro. In tal caso è possibile usare la versione del quarta il [ `Scale` ](https://developer.xamarin.com/api/member/SkiaSharp.SKCanvas.Scale/p/System.Single/System.Single/System.Single/System.Single/) metodo, che include due parametri aggiuntivi per specificare il centro della scala:
...
Questa versione del quarta il `Scale` metodo è una scelta rapida. È possibile visualizzarne il funzionamento, sostituendo il `Scale` metodo in questo codice con quanto segue:
...
A questo punto, che li precedono `Scale` chiamata di aggiungere un altro `Translate` chiamata con valori di centratura:
...
Il codice visualizza anche la stella dieci volte più, ogni volta che diminuendo il ridimensionamento fattore del 10% e progressivamente modificando il colore da rosso a blu:
