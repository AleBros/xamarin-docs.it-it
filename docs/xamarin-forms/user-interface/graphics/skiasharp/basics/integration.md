...
Il `Tapped` gestore per il `TapGestureRecognizer` oggetto semplicemente attiva o disattiva il valore di un campo booleano e chiama le [ `InvalidateSurface` ](xref:SkiaSharp.Views.Forms.SKCanvasView.InvalidateSurface) metodo di `SKCanvasView`:
...
Il metodo statico [ `SKColor.FromHsv` ](xref:SkiaSharp.SKColor.FromHsv(System.Single,System.Single,System.Single,System.Byte)) metodo crea un `SKColor` valore basato sul modello di Tonalità-saturazione-valore simile:
