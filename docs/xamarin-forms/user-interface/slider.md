...
- [`ThumbColor`](xref:Xamarin.Forms.Slider.ThumbColorProperty) rappresenta il colore del cursore. Questa proprietà non è supportata sulla piattaforma Windows universale.
- [`ThumbImage`](xref:Xamarin.Forms.Slider.ThumbImageProperty) è l'immagine da utilizzare per il controllo thumb, typu [ `FileImageSource` ](xref:Xamarin.Forms.FileImageSource). Questa proprietà non è supportata sulla piattaforma Windows universale.
...
Ecco il programma in esecuzione in iOS, Android e Universal Windows Platform (UWP) i dispositivi:
...
L'impostazione `Maximum` su 20 non costituisce un problema perché è superiore all'impostazione predefinita `Minimum` impostazione pari a 0. Quando `Minimum` è impostato, il valore è minore di `Maximum` pari a 20.
...
L'implementazione di Android `Slider` si basa su Android [ `SeekBar` ](https://developer.xamarin.com/api/type/Android.Widget.SeekBar/) e imposta sempre la [ `Max` ](https://developer.xamarin.com/api/property/Android.Widget.ProgressBar.Max/) proprietà a 1000. Ciò significa che il `Slider` in Android ha solo 1.001 valori discreti. Se si imposta il `Slider` per avere una `Minimum` pari a 0 e un `Maximum` pari a 5000, quindi come le `Slider` manipolato, il `Value` proprietà presenta valori pari a 0, 5, 10, 15 e così via.

### <a name="the-uwp-implementation"></a>L'implementazione di UWP

L'implementazione di UWP del `Slider` si basa su UWP [ `Slider` ](/uwp/api/windows.ui.xaml.controls.slider) controllo. Il `StepFrequency` proprietà dell'App UWP `Slider` è impostato per la differenza tra le `Maximum` e `Minimum` proprietà diviso per 10, ma non maggiore di 1.
...
Il `StringFormat` del componente di `Binding` estensione di markup viene impostata per un formato di "F2" per visualizzare due posizioni decimali. (Stringa di formattazione i data binding è descritto nell'articolo [formattazione della stringa](~/xamarin-forms/app-fundamentals/data-binding/string-formatting.md).) Tuttavia, la versione UWP del programma è limitata ai valori di 0, 0.1, 0.2,... 0.9 e 1.0. Si tratta di un risultato diretto dell'implementazione di App UWP `Slider` come descritto in precedenza nella sezione [differenze di implementazione della piattaforma](#implementations).
