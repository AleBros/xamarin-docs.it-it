...
#### <a name="mm0000-unexpected-error---please-file-a-bug-report-at-httpsgithubcomxamarinxamarin-maciosissuesnew"></a>MM0000: errore imprevisto. inviare un report sui bug inhttps://github.com/xamarin/xamarin-macios/issues/new
...
* Log di compilazione completi, con il livello di dettaglio massimo, ad esempio `-v -v -v -v` negli **argomenti aggiuntivi MMP**;
...
#### <a name="mm0001-this-version-of-xamarinmac-requires-mono-0-the-current-mono-version-is-1-please-update-the-monoframework-from-httpmono-projectcomdownloads"></a>MM0001: questa versione di Novell. Mac richiede mono {0} (la versione mono corrente è {1} ). Aggiornare mono. Framework dahttp://mono-project.com/Downloads
...
#### <a name="mm0008-you-should-provide-one-root-assembly-only-found-0-assemblies-1"></a>MM0008: è necessario fornire solo un assembly radice, {0} assembly trovati:' {1} '
...
#### <a name="mm0056-cannot-find-xcode-in-any-of-our-default-locations-please-install-xcode-or-pass-a-custom-path-using---sdkrootpath"></a>MM0056: non è possibile trovare Xcode nei percorsi predefiniti. Installare Xcode o passare un percorso personalizzato usando--sdkroot =\<path>
...
Quando è in esecuzione `--runregistrar` , deve essere fornito almeno un assembly radice.

<a name="MM0131"></a>

#### <a name="mm0131-product-assembly-0-not-found-in-assembly-list-1"></a>MM0131: l'assembly del prodotto ' {0} ' non è stato trovato nell'elenco di assembly:' {1} '

Quando è in esecuzione `--runregistrar` , l'elenco di assembly deve includere l'assembly del prodotto, Novell. Mac, XamMac.
...
#### <a name="mm0133-found-more-than-1-assembly-matching-0-choosing-first-1"></a>MM0133: sono stati trovati più di un assembly corrispondente a' {0} ' che sceglie First:' {1} '
...
#### <a name="mm0135-did-not-link-system-framework-0-referenced-by-assembly-1-because-it-was-introduced-in-2-3-and-were-using-the-2-4-sdk"></a>MM0135: non {0} è stato collegato il Framework di sistema '' (a cui fa riferimento l'assembly ' {1} ') perché è stato introdotto in {2} {3} e si sta usando l' {2} {4} SDK.
...
#### <a name="mm1401-the-required-0-assembly-is-missing-from-the-references"></a>MM1401: l'assembly obbligatorio ' {0} ' non è presente nei riferimenti
...
#### <a name="mm1403-0-1-could-not-be-found-target-framework-0-is-unusable-to-package-the-application"></a>MM1403: {0} ' {1} ' non è stato trovato. Il Framework di destinazione ' {0} ' non è utilizzabile per creare il pacchetto dell'applicazione.
...
#### <a name="mm1406-target-framework-0-is-invalid-when-targeting-xamarinmac-45-net-framwork"></a>MM1406: il Framework di destinazione ' {0} ' non è valido se la destinazione è Novell. Mac 4,5.
...
#### <a name="mm2004-extra-linker-definitions-file-0-could-not-be-located"></a>MM2004: {0} non è stato possibile trovare il file di definizioni del linker aggiuntivo ''.

<a name="MM2005"></a>

#### <a name="mm2005-definitions-from-0-could-not-be-parsed"></a>MM2005: {0} non è stato possibile analizzare le definizioni di ''.

<a name="MM2006"></a>

#### <a name="mm2006-native-library-0-was-referenced-but-could-not-be-found"></a>MM2006: {0} è stato fatto riferimento alla libreria nativa '', che però non è stata trovata.
...
#### <a name="mm2009-referenced-by-01------this-message-is-related-to-mm2006-"></a>MM2009: riferimento da {0} . {1} \* \* Questo messaggio è correlato a MM2006     \*\*
...
#### <a name="mm2016-invalid-tlsprovider-0-option-the-only-valid-value-1-will-be-used"></a>MM2016: opzione TlsProvider non valida `{0}` . Verrà utilizzato l'unico valore valido `{1}` .
...
#### <a name="mm2106-could-not-optimize-the-call-to-blockliteralsetupblockunsafe-in--at-offset--because-"></a>MM2106: Impossibile ottimizzare la chiamata a BlockLiteral. SetupBlock [unsafe] in \* in corrispondenza dell' \* offset \* .

Il linker segnala questo avviso quando non è in grado di ottimizzare una chiamata a `BlockLiteral.SetupBlock` o a `Block.SetupBlockUnsafe` .
...
Prendere `lipo` in considerazione l'uso di per rimuovere le architetture non necessarie in modo permanente dalla libreria di origine. Se l'applicazione non viene pubblicata nell'App Store, questa rimozione può essere disabilitata passando `--optimize=-trim-architectures` come argomenti MMP aggiuntivi.
...
#### <a name="mm4001-the-main-template-could-not-be-expanded-to-0"></a>MM4001: non è stato possibile espandere il modello principale in `{0}` .
...
2. Il registrar non è in grado di trovare il `Invoke` metodo del delegato. Questa operazione non deve essere eseguita, quindi è necessario riportare un [problema](https://github.com/xamarin/xamarin-macios/issues/new) con un progetto di test in modo da poterlo correggere.
...
#### <a name="mm5103-failed-to-compile-error-code---0-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MM5103: non è stato possibile compilare. Codice errore- {0} . Inviare un report sui bug inhttp://bugzilla.xamarin.com
...
#### <a name="mm5202-monoframework-mdk-is-missing-please-install-the-mdk-for-your-monoframework-version-from-httpmono-projectcomdownloads"></a>MM5202: mono. Framework MDK mancante. Installare MDK per la versione di mono. Framework dahttp://mono-project.com/Downloads
...
#### <a name="mm5205-invalid-architecture-0-valid-architectures-are-i386-and-x86_64-when---profilemobile"></a>MM5205: l'architettura '' non è valida {0} . Le architetture valide sono i386 e x86_64 (When--profile = mobile).
...
#### <a name="mm5301-pkg-config-could-not-be-found-please-install-the-monoframework-from-httpmono-projectcomdownloads"></a>MM5301: pkg-config non è stato trovato. Installare Mono. Framework dahttp://mono-project.com/Downloads
...
È possibile forzare il linker a lasciare il registrar dinamico aggiungendo gli `--optimize=-remove-dynamic-registrar` argomenti MMP aggiuntivi nelle opzioni di compilazione Mac del progetto.
