...
Eseguendo l'override del metodo del controller di visualizzazione `UpdateViewConstraints` , lo sviluppatore può testare per verificare se il vincolo necessario è già stato compilato e crearlo, se necessario.
...
Viene eseguito l'accesso alla proprietà di ancoraggio di `NSLayoutGuide` e, se disponibile, viene usata per compilare un nuovo vincolo con la quantità di offset desiderata e il nuovo vincolo viene reso attivo per l'applicazione:
...
Per abilitare la barra degli strumenti semplificata, è necessario eseguire l'override del `ViewWillAppear` metodo di `NSViewController` e l'aspetto è simile al seguente:
...
A seconda della progettazione dell'app, lo sviluppatore potrebbe anche voler integrare l'area della barra del titolo con un controller di visualizzazione accessorio visualizzato immediatamente sotto l'area del titolo o della barra degli strumenti per fornire controlli sensibili al contesto per l'utente in base all'attività in cui sono attualmente coinvolti:
...
Modificare `NSWindowController` e fare in modo che l'aspetto sia simile al seguente:
...
E `LayoutAttribute` che definisce la _posizione in cui_ verrà visualizzato l'accessorio:

```csharp
accessoryView.LayoutAttribute = NSLayoutAttribute.Bottom;
```

Poiché macOS è ora completamente localizzato, `Left` le `Right` `NSLayoutAttribute` proprietà e sono state deprecate e devono essere sostituite con `Leading` e `Trailing` .
...
- Windows verrà automaticamente a schede quando `OrderFront` viene richiamato il metodo.
- Quando viene richiamato il metodo, verrà automaticamente eseguita la tabulazione di Windows `OrderOut` .
...
- Se si tratta di un' `NSDocument` app basata su, molte di queste funzionalità verranno abilitate automaticamente, ad esempio il pulsante più aggiunto alla barra delle schede, senza alcuna azione da parte dello sviluppatore.
- Le `NSDocument` app non basate su possono abilitare il pulsante "più" nel gruppo di schede per aggiungere un nuovo documento eseguendo l'override del `GetNewWindowForTab` metodo di `NSWindowsController` .

Unendo tutti i componenti, il `AppDelegate` di un'app che voleva usare le finestre a schede basate su sistema potrebbe avere un aspetto simile al seguente:
...
Il `NSWindowController` tipo potrebbe essere simile al seguente:
...
Dove la `App` proprietà statica fornisce un collegamento per ottenere l'oggetto `AppDelegate` . Il `SetDefaultDocumentTitle` metodo imposta un nuovo titolo dei documenti in base al numero di nuovi documenti creati.
...
Inoltre, Apple suggerisce l'uso delle visualizzazioni supportate dal livello anziché l'aggiunta di un nuovo `CALayer` come sottolivello, in quanto il sistema gestirà automaticamente diverse impostazioni obbligatorie, ad esempio quelle richieste da una visualizzazione retina.

È possibile abilitare il supporto dei livelli impostando il valore `WantsLayer` di un `NSView` `true` elemento su o all'interno del Interface Builder di Xcode sotto il controllo **Visualizza effetti** controllando il **livello di animazione principale**:
...
Un altro passaggio importante quando si usano le visualizzazioni supportate da un livello in un'app Novell. Mac è l'impostazione `LayerContentsRedrawPolicy` di `NSView` su `OnSetNeedsDisplay` in `NSViewController` . Ad esempio:
...
Quando una visualizzazione viene contrassegnata come Dirty, il sistema controlla la `WantsUpdateLayer` proprietà della vista. Se restituisce `true` `UpdateLayer` , viene chiamato il metodo, in caso contrario `DrawRect` viene chiamato il metodo della vista per aggiornare il contenuto della visualizzazione.
...
- Apple preferisce anche che lo sviluppatore possa comporre la propria interfaccia utente usando visualizzazioni standard come `NSTextField` , quando possibile.

Per usare `UpdateLayer` , creare una classe personalizzata per `NSView` e fare in modo che il codice sia simile al seguente:
...
L'effetto di affollamento è stato ottenuto inviando ogni elemento trascinato al `BeginDraggingSession` metodo di `NSView` come elemento separato in una matrice.

Quando si utilizza un oggetto `NSTableView` o `NSOutlineView` , utilizzare il `PastboardWriterForRow` metodo della `NSTableViewDataSource` classe per avviare l'operazione di trascinamento:
...
Questo codice usa il `TrackEventsMatching` metodo dell'oggetto `NSWindow` a cui è associato l'elemento dell'interfaccia utente per intercettare gli `LeftMouseUp` `LeftMouseDragged` eventi e. Per un `LeftMouseUp` evento, l'elemento dell'interfaccia utente risponde normalmente. Per l' `LeftMouseDragged` evento, l'evento viene passato al `NSWindow` metodo di `PerformWindowDrag` per spostare la finestra sullo schermo.
...
Lo sviluppatore deve sempre usare la nuova `NSView` versione basata sui controlli di visualizzazione del contenitore, ad esempio `NSTableView` . Ad esempio:
...
In questo modo è possibile associare le azioni della riga di tabella personalizzate alle righe specificate della tabella, ad esempio il diritto di scorrimento rapido per eliminare la riga. Per abilitare questo comportamento, eseguire l'override del `RowActions` metodo di `NSTableViewDelegate` :
...
L'oggetto statico `NSTableViewRowAction.FromStyle` viene utilizzato per creare una nuova azione di riga della tabella con gli stili seguenti:
...
- `AutomaticallyAdjustsContentInsets`-Se `true` la visualizzazione a scorrimento gestirà automaticamente `ContentInsets` per lo sviluppatore.

Utilizzando lo `ContentInsets` sviluppatore può modificare l'inizio della visualizzazione a scorrimento per consentire l'inclusione di accessori quali:
...
- **Applicare gli attributi iniziali e finali** : poiché la direzione del testo può variare in base alla lingua dell'utente, utilizzare gli attributi nuovo `Leading` e `Trailing` vincolo anziché gli `Right` attributi e esistenti `Left` . `Leading`e `Trailing` verranno regolate automaticamente in base alla direzione dei linguaggi.
...
- **Usare NSStackViews per aggiungere insieme**  -  `NSStackViews` le visualizzazioni consente lo spostamento e la crescita dei contenuti in modi prevedibili e la modifica delle dimensioni del contenuto in base alla lingua selezionata.
...
Dove `Alignment` e `ImagePosition` vengono impostati in base all'oggetto `UserInterfaceLayoutDirection` del controllo.
...
Il `GetAppearance` metodo statico della `NSAppearance` classe viene usato per ottenere un aspetto denominato dal sistema (in questo caso `NSAppearance.NameVibrantDark` ).
