...
Lo storyboard è un file di risorse (con le estensioni di `.storyboard` ) che viene incluso nel bundle dell'app Novell. Mac quando viene compilato e spedito. Per definire lo storyboard iniziale per l'app, modificare il `Info.plist` file e selezionare l' **interfaccia principale** nella casella a discesa: 
...
Sono stati aggiunti diversi nuovi metodi alla `NSViewController` classe per supportare gli storyboard in MacOS. In particolare, i metodi seguenti usano per rispondere al ciclo di vita della visualizzazione controllata dal controller di visualizzazione specificato:
...
I gli elementi segue attivati consentono di specificare il nome gli elementi segue (tramite la relativa proprietà **Identifier** in Interface Builder) e di attivarli da eventi quali l'utente che fa clic su un pulsante o chiamando il `PerformSegue` metodo nel codice:
...
Facoltativamente, è possibile eseguire l'override del `ShouldPerformSegue` metodo e controllare se il segue viene effettivamente eseguito tramite il codice C#. Per i controller di visualizzazione presentati manualmente, chiamare il `DismissController` metodo per rimuoverli dalla visualizzazione quando non sono più necessari.
...
- Il controller della finestra viene sostituito `ContentViewController` con quello definito dalla destinazione (destinazione) del segue.
...
2. La `Storyboard` proprietà conterrà lo storyboard dal quale è stato caricato il controller della finestra; in `null` caso contrario, se non è stato caricato da uno storyboard.
...
Come i controller di visualizzazione, i controller di finestra implementano i `PerformSegue` `PrepareForSegue` `ShouldPerformSegue` metodi e e possono essere usati come origine di un'operazione segue.
