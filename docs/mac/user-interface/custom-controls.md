...
ms.locfileid: "84573924"
---
# <a name="creating-custom-controls-in-xamarinmac"></a>Creazione di controlli personalizzati in Novell. Mac

Quando si lavora con C# e .NET in un'applicazione Novell. Mac, è possibile accedere agli stessi controlli utente che uno sviluppatore lavora in *Objective-C*, *Swift* e *Xcode* . Poiché Novell. Mac si integra direttamente con Xcode, è possibile usare _Interface Builder_ di Xcode per creare e gestire i controlli utente oppure, facoltativamente, crearli direttamente nel codice C#.
...
In queste situazioni è possibile ereditare facilmente da `NSControl` e creare uno strumento personalizzato che può essere aggiunto all'interfaccia utente dell'app tramite codice C# o tramite la Interface Builder di Xcode. Ereditando dal `NSControl` controllo personalizzato verranno automaticamente incluse tutte le funzionalità standard di un controllo dell'interfaccia utente incorporato (ad esempio `NSButton` ).

Se il controllo dell'interfaccia utente personalizzata Visualizza solo le informazioni (ad esempio uno strumento grafico e grafico personalizzato), è possibile che si desideri ereditare da `NSView` anziché da `NSControl` .
...
Poiché il controllo personalizzato è un'opzione, è necessario un modo per tenere traccia dello stato di attivazione/disattivazione del Commuter. Questa procedura viene gestita con il codice seguente in `NSFlipSwitch` :
...
Successivamente, è necessario eseguire l'override del `DrawRect` metodo e aggiungere le routine grafiche di base per creare il controllo:
...
Si modificherà la rappresentazione visiva per il controllo quando viene modificato lo stato (ad esempio, passando da **on** a **off**). Ogni volta che lo stato viene modificato, è possibile usare il `NeedsDisplay = true` comando per forzare il riprogetto del controllo per il nuovo stato.
...
Gli oggetti che ereditano da `NSControl` (o `NSView` ) dispongono di diversi metodi di override per la gestione dell'input del mouse o della tastiera. Per il controllo di esempio, si vuole capovolgere lo stato del **compartitore quando l'** utente fa **clic sul controllo** con il pulsante sinistro del mouse. Per gestire questo problema, è possibile aggiungere i seguenti metodi di override alla `NSFlipSwitch` classe:
...
Nel codice sopra riportato, viene chiamato il `FlipSwitchState` Metodo (definito in precedenza) per capovolgere lo stato di attivazione/disattivazione dell'opzione nel `MouseDown` metodo. In questo modo verrà forzato il ridisegnato anche il controllo per riflettere lo stato corrente.
...
Facoltativamente, è possibile utilizzare i riconoscitori di movimento per gestire l'interazione dell'utente con il controllo. Rimuovere le sostituzioni aggiunte in precedenza, modificare il `Initialize` metodo e renderlo simile al seguente:
...
Qui viene creato un nuovo oggetto `NSClickGestureRecognizer` e viene chiamato il `FlipSwitchState` metodo per modificare lo stato del commutatore quando l'utente fa clic su di esso con il pulsante sinistro del mouse. Il `AddGestureRecognizer (click)` metodo aggiunge il riconoscitore di movimento al controllo.
...
In primo luogo, viene fornito un `ValueChanged` evento in cui è possibile aggiungere un gestore a nel codice C#, in modo che sia possibile eseguire un'azione quando l'utente modifica lo stato dell'opzione.

In secondo luogo, poiché il controllo personalizzato eredita da `NSControl` , ha automaticamente un' **azione** che può essere assegnata nel Interface Builder di Xcode. Per chiamare questa **azione** quando lo stato viene modificato, viene usato il codice seguente:
...
Con il controllo personalizzato completamente definito, è possibile aggiungerlo all'interfaccia utente dell'app Novell. Mac usando il codice C# o in Interface Builder di Xcode.

Per aggiungere il controllo usando Interface Builder, eseguire prima di tutto una compilazione pulita del progetto Novell. Mac, quindi fare doppio clic sul `Main.storyboard` file per aprirlo in Interface Builder per la modifica:
...
Salvare le modifiche, tornare a Visual Studio per Mac e consentire la sincronizzazione delle modifiche. Modificare il `ViewController.cs` file e fare `ViewDidLoad` in modo che il metodo sia simile al seguente:
...
Qui si risponde all' `ValueChanged` evento definito in precedenza sulla `NSFlipSwitch` classe e si scrive il **valore** corrente quando l'utente fa clic sul controllo.
