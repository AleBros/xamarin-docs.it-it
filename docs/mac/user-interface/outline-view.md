...
Quando si lavora con C# e .NET in un'applicazione Novell. Mac, è possibile accedere alle stesse visualizzazioni struttura che uno sviluppatore lavora in *Objective-C* e *Xcode* . Poiché Novell. Mac si integra direttamente con Xcode, è possibile usare la _Interface Builder_ di Xcode per creare e gestire le visualizzazioni della struttura (oppure, facoltativamente, crearle direttamente nel codice C#).
...
È possibile personalizzare il comportamento di una visualizzazione struttura fornendo una sottoclasse del delegato della visualizzazione struttura ( `NSOutlineViewDelegate` ) per supportare la gestione delle colonne di struttura, digitare per selezionare la funzionalità, la selezione e la modifica delle righe, il rilevamento personalizzato e visualizzazioni personalizzate per singole colonne e righe.
...
> [!IMPORTANT]
> A meno che non si stia gestendo un'applicazione Novell. Mac legacy, è `NSView` consigliabile utilizzare le visualizzazioni struttura basate su `NSCell` viste di tabella basate su. `NSCell`viene considerato Legacy e potrebbe non essere supportato in futuro.
...
Analogamente a qualsiasi altro controllo dell'interfaccia utente di Cocoa, è necessario esporre la visualizzazione struttura e le colonne e le celle al codice C# usando **azioni** e **Outlet** (in base alla funzionalità necessaria).
...
2. Selezionare la visualizzazione struttura dalla **gerarchia dell'interfaccia**, fare clic sul pulsante e trascinarlo sul `ViewController.h` file.
...
Con la visualizzazione struttura progettata in Interface Builder ed esposta tramite un **Outlet**, successivamente è necessario creare il codice C# per popolarlo.

Per prima cosa, creiamo una nuova `Product` classe che contenga le informazioni per le singole righe e i gruppi di sottoprodotti. Nella **Esplora soluzioni**fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**  >  **nuovo file...** Selezionare **General**  >  **classe vuota**generale, immettere `Product` come **nome** e fare clic sul pulsante **nuovo** :
...
Successivamente, è necessario creare una sottoclasse di `NSOutlineDataSource` per fornire i dati per il contorno come richiesto. Nella **Esplora soluzioni**fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**  >  **nuovo file...** Selezionare **General**  >  **classe vuota**generale, immettere `ProductOutlineDataSource` come **nome** e fare clic sul pulsante **nuovo** .
...
Questa classe dispone di archiviazione per gli elementi della visualizzazione struttura ed esegue l'override di `GetChildrenCount` per restituire il numero di righe nella tabella. `GetChild`Restituisce un elemento padre o figlio specifico (come richiesto dalla visualizzazione struttura) e `ItemExpandable` definisce l'elemento specificato come padre o figlio.

Infine, è necessario creare una sottoclasse di `NSOutlineDelegate` per fornire il comportamento per la struttura. Nella **Esplora soluzioni**fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**  >  **nuovo file...** Selezionare **General**  >  **classe vuota**generale, immettere `ProductOutlineDelegate` come **nome** e fare clic sul pulsante **nuovo** .
...
Ciò consentirà all'utente di selezionare una singola riga nella visualizzazione struttura. Restituire `false` per `ShouldSelectItem` per qualsiasi elemento che non si desidera che l'utente sia in grado di selezionare o `false` per ogni elemento se non si desidera che l'utente sia in grado di selezionare elementi.
...
Ciò consentirà all'utente di selezionare una singola riga nella visualizzazione struttura. Restituire `false` per `ShouldSelectRow` per qualsiasi elemento che non si desidera che l'utente sia in grado di selezionare o `false` per ogni elemento se non si desidera che l'utente sia in grado di selezionare elementi.

<a name="Type_to_Select_Row"></a>

## <a name="type-to-select-row"></a>Digitare per selezionare la riga

Se si desidera consentire all'utente di digitare un carattere con la visualizzazione struttura selezionata e selezionare la prima riga con tale carattere, fare doppio clic sul file per `Main.storyboard` aprirlo per la modifica in Interface Builder. Selezionare la visualizzazione struttura nella **gerarchia dell'interfaccia** e selezionare la casella di controllo **tipo seleziona** in **controllo attributi**:
...
Il `GetNextTypeSelectMatch` metodo accetta l' `searchString` oggetto specificato e restituisce l'elemento della prima `Product` che contiene tale stringa `Title` .
...
Questo articolo ha esaminato in dettaglio l'uso delle visualizzazioni struttura in un'applicazione Novell. Mac. Sono stati osservati i diversi tipi e usi delle visualizzazioni struttura, come creare e gestire le visualizzazioni struttura nel Interface Builder di Xcode e come usare le visualizzazioni struttura nel codice C#.
...
- [Visualizzazioni tabelle](~/mac/user-interface/table-view.md)
