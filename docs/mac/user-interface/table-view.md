...
Quando si lavora con C# e .NET in un'applicazione Novell. Mac, è possibile accedere alle stesse visualizzazioni di tabella che uno sviluppatore lavora in *Objective-C* e *Xcode* . Poiché Novell. Mac si integra direttamente con Xcode, è possibile usare la _Interface Builder_ di Xcode per creare e gestire le visualizzazioni di tabella oppure, facoltativamente, crearle direttamente nel codice C#.
...
Una vista tabella consente di visualizzare i dati in formato tabulare contenenti una o più colonne di informazioni in più righe. Le visualizzazioni di tabella vengono visualizzate all'interno delle visualizzazioni di scorrimento ( `NSScrollView` ) e a partire da macOS 10,7, è possibile usare qualsiasi `NSView` anziché cells ( `NSCell` ) per visualizzare sia righe che colonne. Ciò detto, è comunque possibile usare `NSCell` , tuttavia, in genere la sottoclasse `NSTableCellView` e la creazione di righe e colonne personalizzate.
...
> [!IMPORTANT]
> A meno che non si stia gestendo un'applicazione Novell. Mac legacy, è `NSView` consigliabile utilizzare viste di tabella basate su `NSCell` viste di tabella basate su. `NSCell`viene considerato Legacy e potrebbe non essere supportato in futuro.
...
Selezionare ogni colonna nella vista tabella e assegnare alla prima colonna un **titolo** `Product` e il secondo `Details` .

Selezionare una visualizzazione della cella della tabella ( `NSTableViewCell` ) nella **gerarchia dell'interfaccia** e le proprietà seguenti sono disponibili nel **controllo attributi**:
...
Selezionare una cella di visualizzazione tabella (per impostazione predefinita, si tratta di un `NSTextField` ) nella **gerarchia dell'interfaccia** e le proprietà seguenti sono disponibili in **controllo attributi**:
...
Selezionare una visualizzazione della cella della tabella ( `NSTableFieldCell` ) nella **gerarchia dell'interfaccia** e le proprietà seguenti sono disponibili nel **controllo attributi**:
...
Analogamente a qualsiasi altro controllo dell'interfaccia utente di Cocoa, è necessario esporre la visualizzazione tabella e le colonne e le celle al codice C# usando le **azioni** e gli **Outlet** (in base alle funzionalità necessarie).

Il processo è lo stesso per qualsiasi elemento di visualizzazione tabella che si desidera esporre:

1. Passare all' **editor di assistente** e verificare che `ViewController.h` sia selezionato il file: 

    [![](table-view-images/edit11.png "The Assistant Editor")](table-view-images/edit11.png#lightbox)
2. Selezionare la visualizzazione tabella dalla **gerarchia dell'interfaccia**, fare clic sul pulsante e trascinarlo sul `ViewController.h` file.
...
4. Creare **Outlet** per le colonne Tables, oltre `ProductColumn` a `DetailsColumn` : 
...
Con la visualizzazione tabella progettata in Interface Builder ed esposta tramite un **Outlet**, successivamente è necessario creare il codice C# per popolarlo.

Per prima cosa, creiamo una nuova `Product` classe che contenga le informazioni per le singole righe. Nella **Esplora soluzioni**fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**  >  **nuovo file...** Selezionare **General**  >  **classe vuota**generale, immettere `Product` come **nome** e fare clic sul pulsante **nuovo** :

[![](table-view-images/populate01.png "Creating an empty class")](table-view-images/populate01.png#lightbox)

Fare in modo che il file abbia un `Product.cs` aspetto simile al seguente:
...
Successivamente, è necessario creare una sottoclasse di `NSTableDataSource` per fornire i dati per la tabella come richiesto. Nella **Esplora soluzioni**fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**  >  **nuovo file...** Selezionare **General**  >  **classe vuota**generale, immettere `ProductTableDataSource` come **nome** e fare clic sul pulsante **nuovo** .
...
Questa classe dispone di archiviazione per gli elementi della vista tabella ed esegue l'override di `GetRowCount` per restituire il numero di righe nella tabella.

Infine, è necessario creare una sottoclasse di `NSTableDelegate` per fornire il comportamento per la tabella. Nella **Esplora soluzioni**fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**  >  **nuovo file...** Selezionare **General**  >  **classe vuota**generale, immettere `ProductTableDelegate` come **nome** e fare clic sul pulsante **nuovo** .
...
Consente all'utente di ordinare i dati della tabella facendo clic su un'intestazione di colonna. In primo luogo, fare doppio clic sul `Main.storyboard` file per aprirlo per la modifica in Interface Builder. Selezionare la `Product` colonna, immettere `Title` per la **chiave di ordinamento**, `compare:` per il **selettore** e selezionare `Ascending` per l' **ordine**:
...
Il `Sort` metodo consente di ordinare i dati nell'origine dati in base a un `Product` campo della classe specificato in ordine crescente o decrescente. Il metodo sottoposto a override `SortDescriptorsChanged` verrà chiamato ogni volta che l'utilizzo fa clic su un'intestazione di colonna. Viene passato il valore della **chiave** che è stato impostato in Interface Builder e il tipo di ordinamento per la colonna.
...
Se si desidera consentire all'utente di selezionare una singola riga, fare doppio clic sul `Main.storyboard` file per aprirlo per la modifica in Interface Builder. Selezionare la visualizzazione tabella nella **gerarchia dell'interfaccia** e deselezionare la casella di controllo **multipla** in **controllo attributi**:
...
Ciò consentirà all'utente di selezionare una singola riga nella visualizzazione tabella. Restituisce `false` per `ShouldSelectRow` per qualsiasi riga che non si desidera che l'utente sia in grado di selezionare o `false` per ogni riga se non si desidera che l'utente sia in grado di selezionare alcuna riga.
...
Se si desidera consentire all'utente di digitare un carattere con la visualizzazione tabella selezionata e selezionare la prima riga con tale carattere, fare doppio clic sul file per `Main.storyboard` aprirlo per la modifica in Interface Builder. Selezionare la visualizzazione tabella nella **gerarchia dell'interfaccia** e selezionare la casella di controllo **tipo seleziona** in **controllo attributi**:
...
Il `GetNextTypeSelectMatch` metodo accetta l'oggetto specificato `searchString` e restituisce la riga del primo oggetto `Product` con tale stringa `Title` .
...
Il `ShouldReorder` metodo deve restituire `true` per qualsiasi colonna che desidera consentire il trascinamento riordinato in; in caso `newColumnIndex` contrario, restituisce `false` ;
...
Per prima cosa, modificare la `Main.storyboard` Interface Builder di Xcode, selezionare la visualizzazione tabella e aumentare il numero di colonne a tre (3). Modificare quindi il **titolo** della nuova colonna in `Action` :
...
Nello stesso file, modificare la creazione del nuovo delegato di visualizzazione tabella all'interno del `ViewDidLoad` metodo come segue:
...
Modificare quindi il `ProductTableDelegate.cs` file per includere una connessione privata al controller di visualizzazione e prendere il controller come parametro durante la creazione di una nuova istanza del delegato:
...
In questo modo, tutte le configurazioni di visualizzazione di testo eseguite in precedenza nel `GetViewForItem` metodo e le inserisce in un'unica posizione richiamabile (poiché l'ultima colonna della tabella non include una visualizzazione di testo ma un pulsante).
...
Esaminiamo più dettagliatamente alcune sezioni del codice. Prima di tutto, se viene `NSTableViewCell` eseguita un'azione nuova creata in base al nome della colonna. Per le prime due colonne (**Product** e **Details**), `ConfigureTextField` viene chiamato il nuovo metodo.

Per la colonna **azione** , viene creato un nuovo oggetto che `NSButton` viene aggiunto alla cella come vista secondaria:
...
La proprietà del pulsante `Tag` viene utilizzata per conservare il numero della riga attualmente in fase di elaborazione. Questo numero verrà usato in un secondo momento quando l'utente richiede l'eliminazione di una riga nell'evento del pulsante `Activated` :
...
Per la colonna **Action** , tutte le viste secondarie vengono analizzate fino a quando non `NSButton` viene trovato, quindi la `Tag` proprietà viene aggiornata in modo da puntare alla riga corrente.
...
Questo articolo ha esaminato in dettaglio l'uso delle visualizzazioni di tabella in un'applicazione Novell. Mac. Sono stati osservati i diversi tipi e usi delle visualizzazioni di tabella, come creare e gestire le visualizzazioni di tabella nel Interface Builder di Xcode e come usare le visualizzazioni di tabella nel codice C#.
