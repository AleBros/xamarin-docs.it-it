...
In primo luogo, l' `[Register("PersonModel")]` attributo registra la classe e la espone a Objective-C. Quindi, la classe deve ereditare da `NSObject` o da una sottoclasse che eredita da `NSObject` . in questo modo viene aggiunto un metodo di base che consente alla classe di essere conforme a KVC. Successivamente, l' `[Export("Name")]` attributo espone la `Name` proprietà e definisce il valore della chiave che verrà usato in un secondo momento per accedere alla proprietà tramite le tecniche KVC e KVO.

Infine, per poter essere in grado di modificare il valore della proprietà in base al valore della proprietà, è necessario che la funzione di accesso incapsulare le modifiche apportate al relativo valore nelle `WillChangeValue` `DidChangeValue` chiamate al metodo e (specificando la stessa chiave dell' `Export` attributo).  Ad esempio:
...
Il `ValueForKey` metodo restituisce il valore per la chiave specificata (come `NSString` ), relativo all'istanza della classe KVC che riceve la richiesta. Se, ad esempio, `Person` è un'istanza della `PersonModel` classe definita in precedenza:
...
Analogamente, `SetValueForKey` imposta il valore per la chiave specificata (come `NSString` ) rispetto all'istanza della classe KVC che riceve la richiesta. Di nuovo, usando un'istanza della `PersonModel` classe, come illustrato di seguito:
...
Usando l'osservazione chiave-valore (KVO), è possibile aggiungere un Observer a una chiave specifica di una classe conforme a KVC e ricevere una notifica ogni volta che il valore della chiave viene modificato (usando tecniche KVC o accedendo direttamente alla proprietà specificata nel codice C#). Ad esempio:
...
A questo punto, ogni volta `Name` `Person` che viene modificata la proprietà dell'istanza della `PersonModel` classe, il nuovo valore viene scritto nella console.
...
Le sezioni seguenti illustrano come è possibile usare una codifica chiave-valore e una classe conforme all'osservazione del valore chiave per associare i dati agli elementi dell'interfaccia utente nel Interface Builder di Xcode, anziché leggere e scrivere valori usando il codice C#. In questo modo si separa il _modello di dati_ dalle viste usate per visualizzarle, rendendo l'applicazione Novell. Mac più flessibile e facile da gestire. Si riduce inoltre significativamente la quantità di codice da scrivere.
...
Innanzitutto, poiché un dipendente potrebbe essere un responsabile, abbiamo usato un `NSArray` (in particolare un oggetto, in `NSMutableArray` modo che i valori possano essere modificati) per consentire ai dipendenti di essere collegati:
...
1. È stato usato un `NSMutableArray` anziché una matrice o una raccolta C# standard, perché questo è un requisito per l'associazione dei dati ai controlli AppKit, ad esempio le **visualizzazioni tabella**, le **visualizzazioni struttura** e le **raccolte**.
2. È stata esposta la matrice di dipendenti eseguendone il cast a un oggetto `NSArray` per data binding e il relativo nome in formato C# `People` è stato modificato,, in uno che data binding prevede, `personModelArray` nel formato **{class_name} matrice** (si noti che il primo carattere è stato reso minuscolo).
...
Che consentono ai controller di richiedere e modificare i dati visualizzati. Come esposto `NSArray` sopra, questi hanno una convenzione di denominazione molto specifica (che differisce dalle convenzioni di denominazione tipiche di C#):
...
Infine, poiché la `Icon` proprietà si basa sul valore della `isManager` proprietà, le modifiche apportate alla `isManager` proprietà potrebbero non essere riflesse nell'oggetto `Icon` per gli elementi dell'interfaccia utente associati ai dati (durante KVO):
...
Si noti che, oltre alla propria chiave, la `isManager` funzione di accesso invia anche `WillChangeValue` i `DidChangeValue` messaggi e per la chiave, in `Icon` modo che venga visualizzata anche la modifica.
...
Con il modello di dati definito, viene ora esaminato un semplice esempio di data binding nella Interface Builder di Xcode. Ad esempio, aggiungere un modulo all'applicazione Novell. Mac che può essere usata per modificare l'oggetto definito in `PersonModel` precedenza. Verranno aggiunti alcuni campi di testo e una casella di controllo per visualizzare e modificare le proprietà del modello.

Prima di tutto, aggiungere un nuovo **controller di visualizzazione** al file **Main. Storyboard** nel Interface Builder e denominare la relativa classe `SimpleViewController` :
...
A questo punto, quando viene caricata la vista, creare un'istanza del `PersonModel` e popolarla con il codice seguente:
...
2. Selezionare la casella **associa a** e selezionare **Simple View Controller** nell'elenco a discesa. Immettere quindi `self.Person.Name` il **percorso della chiave**:
...
3. Selezionare il campo di testo **occupazione** , selezionare la casella **associa a** e selezionare **Simple View Controller** nell'elenco a discesa. Immettere quindi `self.Person.Occupation` il **percorso della chiave**:
...
4. Selezionare la casella di controllo **Employee è a Manager** e selezionare la casella **Bind to** e selezionare **Simple View Controller** nell'elenco a discesa. Immettere quindi `self.Person.isManager` il **percorso della chiave**:
...
5. Selezionare il campo **numero di dipendenti gestiti** del testo e selezionare la casella **associa a** e selezionare **Simple View Controller** nell'elenco a discesa. Immettere quindi `self.Person.NumberOfEmployees` il **percorso della chiave**:
...
7. Selezionare l'etichetta **numero di dipendenti gestiti** , espandere la couvertura **nascosta** e selezionare la casella **associa a** e selezionare **Simple View Controller** nell'elenco a discesa. Immettere quindi `self.Person.isManager` il **percorso della chiave**:
...
Se si esegue l'applicazione, i valori della `Person` Proprietà compileranno automaticamente il modulo:
...
Tutte le modifiche apportate dagli utenti al modulo verranno riscritte `Person` nella proprietà del controller di visualizzazione. Ad esempio, la deselezione di **Employee è un gestore** che aggiorna l' `Person` istanza di `PersonModel` e il campo di testo e l'etichetta **gestita dei dipendenti** vengono nascosti automaticamente (tramite Data Binding):
...
Prima di tutto, aggiungere un nuovo **controller di visualizzazione** al file **Main. Storyboard** nel Interface Builder e denominare la relativa classe `TableViewController` :
...
Come per la `PersonModel` classe precedente nella sezione [definizione del modello di dati](#Defining_your_Data_Model) , abbiamo esposto quattro metodi pubblici appositamente denominati in modo che il controller dell'array e leggano e scrivano i dati dalla raccolta di `PersonModels` .
...
3. Immettere `PersonModel` per il **nome della classe**, fare clic sul pulsante con il **segno più** e aggiungere tre chiavi. Assegnare loro un nome `Name` `Occupation` e `isManager` :
...
5. Passare al controllo **Binding** e in matrice di **contenuto** Selezionare **associa a** e **controller di visualizzazione tabella**. Immettere il **percorso** di una chiave del modello `self.personModelArray` :

    ![Immissione di un percorso della chiave](databinding-images/table06.png "Immissione di un percorso della chiave")
6. In questo modo il controller di array viene associato alla matrice di `PersonModels` esposta sul controller di visualizzazione.
...
4. `objectValue`è l'oggetto corrente `PersonModel` nella matrice gestita dal controller dell'array.
...
Prima di tutto, aggiungere un nuovo **controller di visualizzazione** al file **Main. Storyboard** nel Interface Builder e denominare la relativa classe `OutlineViewController` :
...
Come per la `PersonModel` classe precedente nella sezione [definizione del modello di dati](#Defining_your_Data_Model) , abbiamo esposto quattro metodi pubblici appositamente denominati in modo che il controller dell'albero e leggano e scrivano i dati dalla raccolta di `PersonModels` .
...
5. Nella sezione **controller albero** immettere `personModelArray` per **elementi figlio**, immettere `NumberOfEmployees` sotto il **conteggio** e immettere `isEmployee` in **foglia**:
...
7. Passare al **controllo Binding** e in matrice di **contenuto** Selezionare **associa a** e **proprietario del file**. Immettere il **percorso** di una chiave del modello `self.personModelArray` :

    ![Modifica del percorso della chiave](databinding-images/outline06.png "Modifica del percorso della chiave")
8. In questo modo il controller dell'albero viene associato alla matrice di `PersonModels` esposta sul controller di visualizzazione.
...
4. `objectValue`è l'oggetto corrente `PersonModel` nella matrice gestita dal controller dell'albero.
5. Selezionare la **cella di visualizzazione tabella** nella colonna **occupazione** . Nel **controllo delle associazioni** sotto il **valore** di couvertura selezionare **associa a** e **visualizzazione celle tabella**. Immettere `objectValue.Occupation` per il **percorso della chiave del modello**:
...
Se si esegue l'applicazione, il contorno verrà popolato con la matrice di `PersonModels` :
...
La creazione di un errore nelle associazioni dati può causare un _arresto anomalo nativo_ del codice non gestito e causare un errore dell'applicazione Novell. Mac `SIGABRT` :
...
3. Non è stato eseguito il wrapping delle modifiche al valore della funzione di accesso in `WillChangeValue` e le `DidChangeValue` chiamate al metodo (specificando la stessa chiave dell' `Export` attributo).
...
Salvare la modifica, tornare a Visual Studio per Mac per sincronizzarla con Xcode ed eseguire l'applicazione. Quando viene visualizzata la visualizzazione raccolta, l'applicazione si arresterà temporaneamente in modo anomalo con un `SIGABRT` errore (come illustrato nell' **output dell'applicazione** in Visual Studio per Mac) perché `PersonModel` non espone una proprietà con la chiave `Title` :
...
Questo articolo ha esaminato in dettaglio l'uso di data binding e la codifica chiave-valore in un'applicazione Novell. Mac. In primo luogo, è stata esaminata l'esposizione di una classe C# a Objective-C utilizzando la codifica chiave-valore (KVC) e l'osservazione chiave-valore (KVO). Successivamente, è stato illustrato come usare una classe conforme a KVO e i dati lo associano agli elementi dell'interfaccia utente nel Interface Builder di Xcode. Infine, ha mostrato data binding complesse usando **controller di array** e **controller di albero**.
