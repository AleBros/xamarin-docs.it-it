...
> [!NOTE]
> Nota: API che producono `ISerializableObject` direttamente oggetti non devono essere gestiti da un `RepresentationProvider`. Se l'oggetto a cui si desidera visualizzare è **non** un `ISerializableObject`, è possibile gestire wrapping nel `RepresentationProvider`.
