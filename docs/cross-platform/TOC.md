# [Multipiattaforma](index.yml)
## [Introduzione](get-started/index.md)
### [Requirements](get-started/requirements.md)
### [Installazione](get-started/installation/index.md)
#### [Installing Xamarin in Visual Studio on Windows](get-started/installation/windows.md) (Installazione di Xamarin in Visual Studio in Windows)
#### [Installing Visual Studio for Mac](/visualstudio/mac/installation/) (Installazione di Visual Studio per Mac)
#### [Istruzioni di configurazione del firewall di](get-started/installation/firewall.md) Xamarin
#### [Uninstalling Xamarin](get-started/installation/uninstalling-xamarin.md) (Disinstallazione di Xamarin)
### [Introduction to Mobile Development](get-started/introduction-to-mobile-development.md) (Introduzione allo sviluppo per dispositivi mobili)
### [Introduzione al ciclo di vita di sviluppo del software per i dispositivi mobili](get-started/introduction-to-mobile-sdlc.md)
## [Nozioni di base sulle applicazioni](app-fundamentals/index.md)
### [Accessibilità](app-fundamentals/accessibility.md)
### [Creazione di applicazioni multipiattaforma](app-fundamentals/building-cross-platform-applications/index.md)
#### [Panoramica](app-fundamentals/building-cross-platform-applications/part-0-overview.md)
#### [Parte 1 - Informazioni sulla piattaforma per dispositivi mobili Xamarin](app-fundamentals/building-cross-platform-applications/part-1-understanding-the-xamarin-mobile-platform.md)
#### [Parte 2 - Architettura](app-fundamentals/building-cross-platform-applications/part-2-architecture.md)
#### [Parte 3- Configurazione di una soluzione multipiattaforma Xamarin](app-fundamentals/building-cross-platform-applications/part-3-setting-up-a-xamarin-cross-platform-solution.md)
#### [Parte 4 - Gestione di più piattaforme](app-fundamentals/building-cross-platform-applications/part-4-platform-divergence-abstraction-divergent-implementation.md)
#### [Parte 5 - Strategie pratiche di condivisione del codice](app-fundamentals/building-cross-platform-applications/part-5-practical-code-sharing-strategies.md)
#### [Parte 6 - Test e approvazioni per App Store](app-fundamentals/building-cross-platform-applications/part-6-testing-and-app-store-approvals.md)
#### [Case Study: Tasky](app-fundamentals/building-cross-platform-applications/case-study-tasky.md)
### [Opzioni di condivisione del codice](app-fundamentals/code-sharing.md)
### [Librerie di classi portabili](app-fundamentals/pcl.md)
### [Progetti condivisi](app-fundamentals/shared-projects.md)
### [.NET Standard](app-fundamentals/net-standard.md)
### [Pacchetti NuGet (automatizzati)](app-fundamentals/nuget-multiplatform-libraries/index.md)
#### [Progetti di libreria esistenti](app-fundamentals/nuget-multiplatform-libraries/existing-library.md)
#### [Nuova libreria multipiattaforma](app-fundamentals/nuget-multiplatform-libraries/single-codebase.md)
#### [Nuove librerie specifiche della piattaforma](app-fundamentals/nuget-multiplatform-libraries/platform-specific.md)
#### [Guida ai metadati](app-fundamentals/nuget-multiplatform-libraries/metadata.md)
### [Pacchetti NuGet (manuali)](app-fundamentals/nuget-manual.md)
### [Localizzazione](app-fundamentals/localization.md)
### [Transport Layer Security (TLS)](app-fundamentals/transport-layer-security.md)
## [Funzionalità della piattaforma](platform/index.md)
### [Panoramica del supporto asincrono](platform/async.md)
### [Funzionalità del linguaggio C# 6](platform/csharp-six.md)
### [F#](platform/fsharp/index.md)
#### [Introduzione](platform/fsharp/overview.md)
#### [Esempi di F#](platform/fsharp/samples.md)
### [Visual Basic.NET](platform/visual-basic/index.md)
#### [Xamarin iOS e Android](platform/visual-basic/native-apps.md)
#### [Xamarin.Forms](platform/visual-basic/xamarin-forms.md)
### [Modelli HTML Razor](platform/razor-html-templates/index.md)
## [Dati e servizi cloud](data-cloud/index.md)
### [Servizi connessi](data-cloud/connected-services.md)
### [Azure Active Directory](data-cloud/active-directory/index.md)
#### [Introduzione](data-cloud/active-directory/get-started/index.md)
##### [Passaggio 1. Registrare](data-cloud/active-directory/get-started/register.md)
##### [Passaggio 2. Configurare](data-cloud/active-directory/get-started/configure.md)
#### [Accesso all'API Graph](data-cloud/active-directory/graph.md)
### [App per dispositivi mobili di Azure](data-cloud/mobile-apps.md)
### [Introduzione ai servizi Web](data-cloud/web-services/index.md)
#### [Procedura dettagliata - Utilizzo di WCF](data-cloud/web-services/walkthrough-working-with-wcf.md)

## [Distribuzione e test](deploy-test/index.md)
### [Configurazione personalizzata del linker](deploy-test/linker.md)
### [Prestazioni](deploy-test/memory-perf-best-practices.md)
### [Installazione di NUnit 2.6.4 tramite NuGet](deploy-test/installing-nunit-using-nuget.md)
### [Debug di più processi](deploy-test/multi-process-debugging.md)
## [Piattaforma Apple](macios/index.md)
### [Gestione degli account Apple](macios/apple-account-management.md)
### [API unificata](macios/unified/index.md)
#### [Panoramica](macios/unified/overview.md)
#### [Aggiornare app esistenti](macios/unified/updating-apps.md)
#### [Aggiornare app iOS esistenti](macios/unified/updating-ios-apps.md)
#### [Aggiornare app Mac esistenti](macios/unified/updating-mac-apps.md)
#### [Aggiornare app Xamarin.Forms esistenti](macios/unified/updating-xamarin-forms-apps.md)
#### [Eseguire la migrazione di un binding all'API unificata](macios/unified/update-binding.md)
#### [Suggerimenti per l'aggiornamento del codice per l'API unificata](macios/unified/updating-tips.md)
### [Binding di Objective-C](macios/binding/index.md)
#### [Panoramica](macios/binding/overview.md)
#### [Binding di librerie Objective-C](macios/binding/objective-c-libraries.md)
#### [Informazioni di riferimento sul binding dei tipi](macios/binding/binding-types-reference.md)
#### [Objective Sharpie](macios/binding/objective-sharpie/index.md)
##### [Introduzione](macios/binding/objective-sharpie/get-started.md)
##### [Strumenti e comandi](macios/binding/objective-sharpie/tools.md)
##### [Funzionalità](macios/binding/objective-sharpie/platform/index.md)
###### [ApiDefinitions e StructsAndEnums](macios/binding/objective-sharpie/platform/apidefinitions-structsandenums.md)
###### [Binding di framework nativi](macios/binding/objective-sharpie/platform/native-frameworks.md)
###### [Attributi Verify](macios/binding/objective-sharpie/platform/verify.md)
##### [Esempi](macios/binding/objective-sharpie/examples/index.md)
###### [Xcode](macios/binding/objective-sharpie/examples/xcode.md)
###### [CocoaPod](macios/binding/objective-sharpie/examples/cocoapod.md)
###### [Esempio avanzato (manuale)](macios/binding/objective-sharpie/examples/advanced.md)
##### [Cronologia delle versioni](macios/binding/objective-sharpie/releases.md)
#### [Risoluzione dei problemi](macios/binding/troubleshooting.md)
### [Riferimenti nativi](macios/native-references.md)
### [Tipi nativi](macios/nativetypes.md)
### [Considerazioni sulle piattaforme a 32 e a 64 bit](macios/32-and-64/index.md)
#### [Aggiornamento delle app unificate Xamarin.Mac a 64 bit](macios/32-and-64/mac-64-bit.md)
### [Utilizzo di tipi nativi nelle app multipiattaforma](macios/native-types-cross-platform.md)
### [Stack HttpClient e selettore dell'implementazione di SSL/TLS](macios/http-stack.md)
## [Meccanismi interni](internals/index.md)
### [Assembly disponibili](internals/available-assemblies.md)
## [Risoluzione dei problemi](troubleshooting/index.md)
### [Domande frequenti](troubleshooting/questions/index.md)
#### [È possibile usare Visual Studio 2017 Release Candidate con Xamarin?](troubleshooting/questions/visualstudio-2017-rc.md)
#### [Come è possibile visualizzare le librerie supportate in una libreria di classi portabile (PCL)?](troubleshooting/questions/pcl-support-libraries.md)
#### [API Reflection per librerie PCL](troubleshooting/questions/pcl-reflection.md)
#### [Case study per librerie PCL: come è possibile risolvere i problemi correlati a System.Diagnostics.Tracing per il pacchetto NuGet Microsoft TPL Dataflow?](troubleshooting/questions/pcl-case-study.md)
#### [Come si aggiorna NuGet?](troubleshooting/questions/nuget-update.md)
#### [Come si effettua il downgrade di un pacchetto NuGet?](troubleshooting/questions/nuget-package-downgrade.md)
#### [Errore di pacchetti mancanti dopo l'aggiornamento di pacchetti Nuget](troubleshooting/questions/nuget-packages-missing.md)
#### [Unificazione dei componenti di Google Play Services e NuGet](troubleshooting/questions/gps-components-nuget.md)
#### [In quale posizione sono archiviati i componenti nel computer?](troubleshooting/questions/component-storage.md)
#### [Dove si possono trovare informazioni sulla versione e i log?](troubleshooting/questions/version-logs.md)
#### [Quando e come è opportuno registrare un report sui bug?](troubleshooting/questions/howto-file-bug.md)
#### [Perché Jenkins non è supportato da Xamarin?](troubleshooting/questions/xamarin-jenkins.md)
#### [Quali impostazioni di progetto sono richieste per il debugger?](troubleshooting/questions/debugger-settings.md)
### [Aggiornamento dei riferimenti ai componenti per NuGet](troubleshooting/component-nuget.md)
### [Opzioni di supporto](troubleshooting/support-options.md)
### [Domande sulle licenze legacy](troubleshooting/legacy-licenses/index.md)
#### [Gestione dei team per la piattaforma Xamarin](troubleshooting/legacy-licenses/team-management.md)
#### [Come si modifica l'indirizzo di posta elettronica dell'account?](troubleshooting/legacy-licenses/change-email.md)
#### [Come si risincronizzano manualmente le licenze Xamarin?](troubleshooting/legacy-licenses/resync-licenses.md)
#### [Alcuni errori specifici per le licenze](troubleshooting/legacy-licenses/licensing-errors.md)
#### [Perché non è possibile accedere a Xamarin in Visual Studio o Visual Studio per Mac?](troubleshooting/legacy-licenses/login-troubleshooting.md)
## [Esempi](samples/index.yml)
