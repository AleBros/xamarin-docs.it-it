...
description: Questo documento vengono descritti i passaggi che devono essere seguiti per aggiornare un'app Xamarin.Mac dall'API classica all'API unificata.
ms.prod: xamarin
...
Per un'applicazione per dispositivi mobili Xamarin.Mac che è stata convertita in Unified API, lo sviluppatore deve comunque consentono di creare l'applicazione per computer a 64 bit delle opzioni dell'applicazione. Vedere il **abilitazione Bit 64 compilazioni di App Xamarin.Mac** del [considerazioni relative alla piattaforma a 32/64 bit](~/cross-platform/macios/32-and-64/index.md) documenti per istruzioni dettagliate sull'abilitazione di a 64 bit.
