...
description: In questo documento viene fornita una panoramica dei diversi modi per creare associazioni C# per il codice Objective-C, incluse le associazioni della riga di comando, i progetti di binding e l'obiettivo Sharpie. Viene inoltre illustrato il funzionamento dell'associazione.
ms.prod: xamarin
...
1. Scrivere una "definizione API" in C# per descrivere il modo in cui l'API nativa viene esposta in .NET e il modo in cui viene eseguito il mapping all'oggetto Objective-C sottostante. Questa operazione viene eseguita usando costrutti C# standard come `interface` e diversi **attributi** di associazione (vedere questo [semplice esempio](~/cross-platform/macios/binding/objective-c-libraries.md#Binding_an_API)).

2. Una volta scritta la "definizione API" in C#, viene compilata per produrre un assembly "binding". Questa operazione può essere eseguita dalla [**riga di comando**](#command-line-bindings) o tramite un [**progetto di binding**](#bindingproject) in Visual Studio per Mac o Visual Studio.
...
È possibile usare `btouch-native` per Novell. iOS (o `bmac-native` se si usa Novell. Mac) per compilare direttamente le associazioni. Funziona passando le definizioni dell'API C# create manualmente (o usando l'obiettivo Sharpie) allo strumento da riga di comando ( `btouch-native` per iOS o `bmac-native` per Mac).
...
In secondo luogo, è necessario creare il tipo C#. È probabile che si desideri inserire questo oggetto in uno spazio dei nomi; Poiché Objective-C non supporta gli spazi dei nomi, è necessario usare l' `[Register]` attributo per modificare il nome del tipo che Novell. iOS registrerà con il runtime di Objective-c. Il tipo C# deve anche ereditare da [Foundation. NSObject](xref:Foundation.NSObject):
...
Quinto, fornire i metodi per ognuno dei selettori dichiarati nel passaggio 3. Che utilizzeranno `objc_msgSend()` per richiamare il selettore sull'oggetto nativo. Si noti l'uso di [Runtime. GetNSObject ()](xref:ObjCRuntime.Runtime.GetNSObject*) per convertire un oggetto `IntPtr` in un `NSObject` tipo (Sub-) tipizzato in modo appropriato. Se si desidera che il metodo sia chiamabile dal codice Objective-C, il membro *deve* essere **virtuale**.
