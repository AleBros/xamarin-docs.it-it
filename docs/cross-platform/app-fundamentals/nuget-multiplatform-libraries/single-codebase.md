...
description: Questo documento viene descritto come creare una libreria multipiattaforma per l'utilizzo con NuGet. Questa tecnica è utile per la logica di business e gli algoritmi che possono essere espresso interamente nella libreria di classi Base .NET e pertanto verranno eseguito su tutte le piattaforme di destinazione senza codice specifico della piattaforma.
ms.prod: xamarin
