...
Inizia con una progettazione di alto livello che può essere implementata tra le piattaforme di destinazione. Prendere in considerazione i vincoli dell'interfaccia utente della piattaforma specifico. Ad esempio, un `TabBarController` in iOS può visualizzare più di cinque pulsanti, mentre l'equivalente Windows Phone può essere visualizzato fino a quattro.
...
Il diagramma classi seguente mostra le classi raggruppate per livello. La classe `SQLiteConnection` è un codice standard del pacchetto sqlite-NET. Le altre classi sono codice personalizzato per Tasky. Le classi `TaskItemManager` e `TaskItem` rappresentano l'API esposta alle applicazioni specifiche della piattaforma.
...
L'utilizzo di spazi dei nomi per separare i livelli consente di gestire i riferimenti tra ogni livello. È necessario che i progetti specifici della piattaforma includano solo un'istruzione `using` per il livello business. Il livello di accesso ai dati e il livello dati devono essere incapsulati dall'API esposta da `TaskItemManager` al livello business.
...
Tasky si basa sul pacchetto sqlite-net NuGet (pubblicato da Frank Kreuger) per incorporare il codice SQLite-NET che fornisce un'interfaccia di database ORM (Object-Relational Mapping). La classe `TaskItemDatabase` eredita da `SQLiteConnection` e aggiunge i metodi di creazione, lettura, aggiornamento ed eliminazione (CRUD) necessari per la lettura e la scrittura dei dati in SQLite. Si tratta di un'implementazione standard semplice di metodi CRUD generici che possono essere riutilizzati in altri progetti.

Il `TaskItemDatabase` è un singleton, assicurando che tutti gli accessi vengano eseguiti sulla stessa istanza. Viene utilizzato un blocco per impedire l'accesso simultaneo da più thread.
...
Un esempio dei metodi generici nel livello dati che usano l'interfaccia è il metodo `GetItem<T>`:
...
Un [blocco](https://msdn.microsoft.com/library/c5kehkcz(v=vs.100).aspx) viene implementato all'interno della classe `TaskItemDatabase` per impedire l'accesso simultaneo al database. Ciò consente di garantire che l'accesso simultaneo da thread diversi venga serializzato. in caso contrario, un componente dell'interfaccia utente potrebbe tentare di leggere il database nello stesso momento in cui è stato aggiornato da un thread in background. Di seguito è riportato un esempio di come viene implementato il blocco:
...
La maggior parte del codice del livello dati può essere riutilizzata in altri progetti. L'unico codice specifico dell'applicazione nel livello è la chiamata `CreateTable<TaskItem>` nel costruttore di `TaskItemDatabase`.

 <a name="Data_Access_Layer_(DAL)" />

### <a name="data-access-layer-dal"></a>Livello di accesso ai dati (DAL)

La classe `TaskItemRepository` incapsula il meccanismo di archiviazione dei dati con un'API fortemente tipizzata che consente di creare, eliminare, recuperare e aggiornare oggetti `TaskItem`.
...
In Tasky il modello è la classe `TaskItem` e `TaskItemManager` implementa il modello di facciata per fornire un'API per la gestione di `TaskItems`.

 <a name="Façade" />

#### <a name="faade"></a>Facciata

 `TaskItemManager` esegue il wrapping del `DAL.TaskItemRepository` per fornire i metodi get, Save e Delete a cui i livelli dell'applicazione e dell'interfaccia utente faranno riferimento.
...
Una volta scritto il codice comune, è necessario compilare l'interfaccia utente per raccogliere e visualizzare i dati esposti. La classe `TaskItemManager` implementa il modello di facciata per fornire un'API semplice per l'accesso al codice dell'applicazione.

Il codice scritto in ogni progetto specifico della piattaforma sarà in genere strettamente associato all'SDK nativo del dispositivo e accederà solo al codice comune usando l'API definita dal `TaskItemManager`. Sono inclusi i metodi e le classi aziendali esposte, ad esempio `TaskItem`.
...
- **EditingSource** : questa classe viene utilizzata per associare elenchi di attività all'interfaccia utente. Poiché `MonoTouch.Dialog` è stato usato per l'elenco attività, è necessario implementare questo helper per abilitare la funzionalità di scorrimento a eliminazione nella `UITableView`. L'operazione di scorrimento rapido è comune in iOS, ma non in Android o Windows Phone, quindi il progetto specifico iOS è l'unico che lo implementa.
- **TaskDialog** : questa classe viene usata per associare una singola attività all'interfaccia utente. Usa l'API di Reflection `MonoTouch.Dialog` per eseguire il wrapping dell'oggetto `TaskItem` con una classe che contiene gli attributi corretti per consentire la formattazione corretta della schermata di input.

La classe `TaskDialog` usa gli attributi `MonoTouch.Dialog` per creare una schermata in base alle proprietà di una classe. La classe ha un aspetto simile al seguente:
...
Si noti che gli attributi `OnTap` richiedono un nome di metodo: questi metodi devono esistere nella classe in cui viene creata la `MonoTouch.Dialog.BindingContext` (in questo caso, la classe `HomeScreen` descritta nella sezione successiva).
...
1. **PopulateTable** : usa il metodo `TaskManager.GetTasks` del livello business per recuperare una raccolta di oggetti `TaskItem` da visualizzare.
...
La funzionalità della schermata dei **Dettagli delle attività** , ad esempio il salvataggio o l'eliminazione di un'attività, deve essere implementata nella classe `HomeScreen`, perché è la posizione in cui viene creata la `MonoTouch.Dialog.BindingContext`. I seguenti metodi di `HomeScreen` supportano la schermata dei dettagli dell'attività:

1. **ShowTaskDetails** : crea un `MonoTouch.Dialog.BindingContext` per eseguire il rendering di una schermata. Crea la schermata di input usando la reflection per recuperare i nomi e i tipi di proprietà dalla classe `TaskDialog`. Altre informazioni, ad esempio il testo della filigrana per le caselle di input, vengono implementate con gli attributi delle proprietà.
2. **SaveTask** : viene fatto riferimento a questo metodo nella classe `TaskDialog` tramite un attributo `OnTap`. Viene chiamato quando viene premuto **Save** e utilizza un `MonoTouch.Dialog.BindingContext` per recuperare i dati immessi dall'utente prima di salvare le modifiche utilizzando `TaskItemManager`.
3. **DeleteTask** : viene fatto riferimento a questo metodo nella classe `TaskDialog` tramite un attributo `OnTap`. USA `TaskItemManager` per eliminare i dati usando la chiave primaria (proprietà ID).
...
Il codice della schermata iniziale definisce i gestori per fare clic sul pulsante e fare clic sugli elementi nell'elenco, nonché popolare l'elenco nel metodo `OnResume` (in modo che rifletta le modifiche apportate nella schermata dei dettagli dell'attività). I dati vengono caricati utilizzando la `TaskItemManager` del livello business e il `TaskListAdapter` dal livello dell'applicazione.

 <a name="Task_Details_Screen" />

#### <a name="task-details-screen"></a>Schermata dei dettagli dell'attività

La schermata Dettagli attività è costituita anche da una sottoclasse `Activity` e un file di layout AXML. Il layout determina la posizione dei controlli di input e la C# classe definisce il comportamento da caricare e salvare `TaskItem` oggetti.
...
Tutti i riferimenti alla libreria PCL si trovano attraverso la classe `TaskItemManager`.
...
ViewModels esegue il wrapping dei dati dalla libreria di classi portabile (`TaskItemManager`) e li presenta in modo che possa essere utilizzato da Silverlight/XAML data binding. Questo è un esempio di comportamento specifico della piattaforma, come descritto nel documento relativo alle applicazioni multipiattaforma.
...
La classe MainPage usa il `TaskListViewModel` per visualizzare i dati usando le funzionalità di data binding di XAML. Il `DataContext` della pagina è impostato sul modello di visualizzazione, che viene popolato in modo asincrono. La sintassi `{Binding}` in XAML determina la modalità di visualizzazione dei dati.

 <a name="TaskDetailsPage" />

#### <a name="taskdetailspage"></a>TaskDetailsPage

Ogni attività viene visualizzata associando il `TaskViewModel` al codice XAML definito in TaskDetailsPage. XAML. I dati dell'attività vengono recuperati tramite il `TaskItemManager` nel livello business.
...
L'applicazione usa la progettazione dell'interfaccia utente standard iOS, ad esempio il pulsante "Aggiungi" posizionato nella barra di spostamento e usando l'icona **più (+)** incorporata. USA anche il comportamento predefinito del pulsante `UINavigationController`' back ' e supporta ' Swipe-to-delete ' nella tabella.
...
Ha descritto il processo usato per progettare i livelli dell'applicazione e ha illustrato quale codice &amp; funzionalità è stata implementata in ogni livello.
