...
Il [libro](https://developer.xamarin.com/r/xamarin-forms/book/) *creazione di app per dispositivi mobili con Xamarin.Forms*
...
Per effettuare chiamate di thread dell'interfaccia utente nel codice condiviso, seguire le [esempio IDispatchOnUIThread](https://www.slideshare.net/follesoe/cross-platform-mobile-apps-using-net) (fornito [ @follesoe ](https://twitter.com/follesoe)). Dichiarare e programmare un `IDispatchOnUIThread` interfaccia nel codice condiviso e quindi implementare le classi specifiche della piattaforma, come illustrato di seguito:
...
Gli sviluppatori di Xamarin.Forms devono utilizzare [ `Device.BeginInvokeOnMainThread` ](~/xamarin-forms/platform/device.md#interact-with-the-ui-from-background-threads) nel codice comune (libreria di classi Portabile o progetti condivisi).
