...
Anche con il motore di database disponibile su tutte le piattaforme, i metodi nativi per accedere al database sono diversi. IOS e Android offrono API incorporate per accedere a SQLite che può essere usato da Novell. iOS o Novell. Android. Tuttavia, l'uso dei metodi SDK nativi non offre la possibilità di condividere codice (ad eccezione delle query SQL, presumendo che siano archiviate come stringhe). Per informazioni dettagliate sulle funzionalità di database Native `CoreData` , cercare nella classe iOS o Android. `SQLiteOpenHelper` poiché queste opzioni non sono multipiattaforma, non rientrano nell'ambito di questo documento.

### <a name="adonet"></a>ADO.NET

Il supporto per Novell. iOS e Novell. Android `System.Data` e `Mono.Data.Sqlite` (vedere la [documentazione](~/ios/data-cloud/system.data.md) di Novell. iOS per altre informazioni).
L'uso di questi spazi dei nomi consente di scrivere codice ADO.NET che funziona su entrambe le piattaforme. Modificare i riferimenti del progetto in modo da includere `System.Data.dll` e `Mono.Data.Sqlite.dll` aggiungere le istruzioni using seguenti al codice:
...
Per prima cosa, la `TodoItem` classe utilizza gli attributi per definire un campo come chiave primaria del database:
...
In questo modo `TodoItem` è possibile creare una tabella con la seguente riga di codice (e nessuna istruzione SQL) in un' `SQLiteConnection` istanza:

```csharp
CreateTable<TodoItem> ();
```

I dati nella tabella possono essere modificati anche con altri metodi in (anche in questo `SQLiteConnection` caso, senza richiedere istruzioni SQL):
...
Sia Novell. iOS che Novell. Android consentono l'accesso file system usando le classi nello `System.IO` spazio dei nomi.
...
- Windows Phone 8 (Silverlight) non consente l'accesso diretto ai file: i file possono essere modificati solo tramite `IsolatedStorage` .
...
`Environment.GetFolderPath`L'uso di consente di eseguire lo stesso codice in iOS e Android, ognuno dei quali restituisce una directory valida basata sulle convenzioni del file System.
...
Per altre informazioni sulle funzionalità del [file System](~/ios/app-fundamentals/file-system.md) specifiche di iOS, vedere il documento relativo all'uso di Novell. iOS. Quando si scrive il codice di accesso ai file multipiattaforma, tenere presente che alcuni file System distinguono tra maiuscole e minuscole e hanno separatori di directory diversi. È consigliabile usare sempre la stessa combinazione di maiuscole e minuscole per i nomi di file e il `Path.Combine()` metodo durante la costruzione di percorsi di file o directory.

### <a name="windowsstorage-for-windows-8-and-windows-10"></a>Windows. storage per Windows 8 e Windows 10

Il capitolo 20 relativo alla *creazione di app per dispositivi mobili con Novell. Forms* [book](https://developer.xamarin.com/r/xamarin-forms/book/) 
 [. Async e I/O di file](https://developer.xamarin.com/r/xamarin-forms/book/chapter20.pdf) includono [esempi per Windows 8.1 e Windows 10](https://github.com/xamarin/xamarin-forms-book-preview-2/tree/master/Chapter20).

Con l'uso di un [`DependencyService`](~/xamarin-forms/app-fundamentals/dependency-service/index.md) è possibile leggere e file su queste piattaforme usando le API supportate:
...
Si tratta del meccanismo predefinito per l'accesso ai file in Windows Phone (Silverlight) implementato in Novell. iOS e Novell. Android per consentire la scrittura di codice di accesso ai file comune. `System.IO.IsolatedStorage`È possibile fare riferimento alla classe tra tutte e tre le piattaforme in un [progetto condiviso](~/cross-platform/app-fundamentals/shared-projects.md).
...
La `WebClient` classe fornisce un'API semplice per recuperare i dati remoti dai server remoti.
...
Il codice per una semplice `WebClient` operazione asincrono è:
...
```
 ...
 `WebClient`dispone inoltre `DownloadFileCompleted` `DownloadFileAsync` di e per il recupero di dati binari.

<a name="HttpWebRequest"></a>

### <a name="httpwebrequest"></a>HttpWebRequest

`HttpWebRequest`offre una maggiore personalizzazione di `WebClient` e, di conseguenza, richiede un maggior numero di codice da usare.
...
Gli strumenti di Novell consentono di utilizzare alcuni servizi di Windows Communication Foundation (WCF). In generale, Novell supporta lo stesso subset lato client di WCF fornito con il runtime di Silverlight. Sono incluse le implementazioni più comuni di codifica e protocollo di WCF: messaggi SOAP con codifica testo sul protocollo di trasporto HTTP utilizzando `BasicHttpBinding` .
...
La chiave è `TaskScheduler.FromCurrentSynchronizationContext()` che riutilizzerà SynchronizationContext. Current del thread che chiama il metodo (in questo caso il thread principale in esecuzione `MainThreadMethod` ) come metodo per eseguire il marshalling delle chiamate a tale thread. Ciò significa che se il metodo viene chiamato sul thread dell'interfaccia utente, l'operazione verrà eseguita `ContinueWith` di nuovo sul thread dell'interfaccia utente.
...
Per eseguire chiamate al thread dell'interfaccia utente nel codice condiviso, seguire l' [esempio IDispatchOnUIThread](https://www.slideshare.net/follesoe/cross-platform-mobile-apps-using-net) (courtesy of [@follesoe](https://twitter.com/follesoe) ). Dichiarare e programmare in un' `IDispatchOnUIThread` interfaccia nel codice condiviso e quindi implementare le classi specifiche della piattaforma, come illustrato di seguito:
...
Gli sviluppatori di Novell. Forms devono usare [`Device.BeginInvokeOnMainThread`](~/xamarin-forms/platform/device.md#interact-with-the-ui-from-background-threads) in codice comune (progetti condivisi o PCL).
