...
Per consentire a un'app di passare a un'icona alternativa, è necessario includere una raccolta di immagini icona nel progetto app Novell. iOS. Queste immagini non possono essere aggiunte al progetto usando il `Assets.xcassets` metodo tipico, ma devono essere aggiunte direttamente alla cartella **Resources** .
...
7. Aggiungere una nuova stringa alla `CFBundleIconFiles` matrice per ogni file di icona lasciando l'estensione e i `@2x` `@3x` suffissi, e così via (esempio `100_icon` ). Ripetere questo passaggio per ogni file che compone il set di icone alternative.
8. Aggiungere una `UIPrerenderedIcon` chiave al `AppIcon2` dizionario, impostare il **tipo** su **booleano** e il valore su **No**.
...
La `ApplicationIconBadgeNumber` proprietà della `UIApplication` classe consente allo sviluppatore di ottenere o impostare il numero di notifica corrente dell'icona dell'app nel Springboard. Il valore predefinito è zero (0). Ad esempio:
