...
Le attività in background e sicure `DidEnterBackground` sono sicure da usare in iOS 6 e iOS 7, con alcune piccole differenze. Esaminiamo questi due tipi di attività in modo più dettagliato.
...
Il processo di registrazione associa un'attività a un identificatore univoco, `taskID` , quindi ne esegue il wrapping nelle `BeginBackgroundTask` chiamate e corrispondenti `EndBackgroundTask` . Per generare l'identificatore, effettuare una chiamata al `BeginBackgroundTask` metodo sull' `UIApplication` oggetto, quindi avviare l'attività a esecuzione prolungata, in genere in un nuovo thread. Quando l'attività viene completata, viene chiamato `EndBackgroundTask` e passato lo stesso identificatore. Questo è importante perché iOS terminerà l'applicazione se una `BeginBackgroundTask` chiamata non ha un corrispondente `EndBackgroundTask` .
...
Si inizia eseguendo l'override del `DidEnterBackground` metodo in `AppDelegate` , in cui viene registrata l'attività tramite `BeginBackgroundTask` come nell'esempio precedente. Successivamente, si genera un nuovo thread e si esegue l'attività a esecuzione prolungata. Si noti che la `EndBackgroundTask` chiamata viene ora eseguita dall'interno dell'attività a esecuzione prolungata, perché il `DidEnterBackground` metodo sarà già stato restituito.

> [!IMPORTANT]
> iOS usa un [meccanismo watchdog](https://developer.apple.com/library/ios/qa/qa1693/_index.html) per garantire che l'interfaccia utente di un'applicazione rimanga reattiva. Un'applicazione che trascorre troppo tempo in non `DidEnterBackground` risponderà nell'interfaccia utente. Avviare le attività per l'esecuzione in background consente `DidEnterBackground` un ritorno tempestivo, mantenendo la risposta dell'interfaccia utente e impedendo al watchdog di terminare l'applicazione.

## <a name="handling-background-task-time-limits"></a>Gestione dei limiti temporali delle attività in background

iOS pone limiti rigidi per quanto tempo può essere eseguita un'attività in background e se la `EndBackgroundTask` chiamata non viene eseguita entro il tempo assegnato, l'applicazione verrà terminata. Tenendo traccia del tempo di background rimanente e usando i gestori di scadenza quando necessario, è possibile evitare che iOS interrompa l'applicazione.

### <a name="accessing-background-time-remaining"></a>Accesso al tempo rimanente in background

Se un'applicazione con attività registrate viene spostata in background, le attività registrate otterranno circa 600 secondi per l'esecuzione. È possibile controllare il tempo necessario per completare l'attività usando la proprietà statica `BackgroundTimeRemaining` della `UIApplication` classe. Il codice seguente ci fornirà il tempo, in secondi, che l'attività in background ha lasciato:
...
l'elaborazione in background di iOS 7 è ottimizzata per la durata della batteria maggiore. In iOS 7, lo sfondo diventa opportunistico: invece di tenere attivo il dispositivo, le attività rispettano il momento in cui il dispositivo passa alla modalità di sospensione e invece eseguono la loro elaborazione in blocchi quando il dispositivo viene riattivato per gestire le chiamate telefoniche, le notifiche, i messaggi di posta elettronica in arrivo e altre interruzioni comuni. Il diagramma seguente illustra in che modo è possibile suddividere un'attività:
...
L' `NSURLSession` API crea una o più sessioni, che a loro volta generano attività per i blocchi Shuttle dei dati correlati attraverso la rete. Le attività vengono eseguite in modo asincrono per trasferire i dati in modo rapido e affidabile. Poiché `NSURLSession` è asincrono, ogni sessione richiede un blocco del gestore di completamento per consentire al sistema e all'applicazione di stabilire quando un trasferimento viene completato.

Per eseguire un trasferimento di rete valido sia in pre-iOS 7 che dopo iOS 7, verificare che `NSURLSession` sia disponibile per i trasferimenti di Accodamento e utilizzare un'attività in background regolare per eseguire il trasferimento in caso contrario:
