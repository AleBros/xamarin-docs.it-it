...
Come installare e iniziare a utilizzare la versione beta Xcode 9 e l'anteprima del supporto di 11 iOS di Xamarin.
...
IOS di Apple 10 include diverse nuove API e servizi che consentono di sviluppare applicazioni con nuove caratteristiche e funzionalità. Le app iOS 10, prevedono nuove funzionalità, ad esempio l'estensione di mappe, i messaggi, telefono e Siri. In questa sezione viene illustrato che per poter sfruttare queste funzionalità in un'app xamarin. IOS.


##  <a name="introduction-to-ios-9iosplatformintroduction-to-ios9indexmd"></a>[Introduzione a iOS 9](~/ios/platform/introduction-to-ios9/index.md)

In questa sezione definisce le modifiche apportate durante l'aggiornamento da iOS 8 e come utilizzare queste funzionalità in un'app xamarin iOS 9.
...
IOS di Apple 8 effettuato un numero elevato di modifiche al sistema operativo da iOS 7. In questo caso, ecco che cosa sono e come utilizzarle.
...
Sulle principali nuove API introdotte in iOS 7, tra cui View Controller assume, miglioramenti alle animazioni UIView UIKit Dynamics e il Kit di testo.
...
Descrizione delle funzionalità introdotte in iOS 6, nonché le visualizzazioni di raccolta, Kit passare, Kit di evento e il Framework di social networking.
...
Grafica e animazione in iOS vengono illustrati i concetti di grafica di base in iOS, ad esempio CoreImage, grafica di base e animazione di base.
...
iOS 9 aggiunge il supporto di multitasking per le due applicazioni in esecuzione nello stesso momento su iPad specifiche hardware. Il multitasking per iPad è supportato tramite le funzionalità seguenti: diapositiva su, doppia visualizzazione & immagine nell'immagine.

## <a name="passkitiosplatformpasskitmd"></a>[PassKit](~/ios/platform/passkit.md)

Passbook è un'app per iPhone e iPod tocca con iOS 6. Archivia e visualizza i codici a barre e altre informazioni per collegare le transazioni cliente sul telefono con il mondo reale. Passa è generati da aziende e inviata al cliente tramite posta elettronica, URL o da un'app di iOS dell'azienda. Passbook archivia organizza tutti i passaggi su un telefono e consente di visualizzare i promemoria passata nella schermata di blocco a seconda della data/ora o la posizione del dispositivo.

Questo documento introduce Passbook, tramite l'API di Kit passare con xamarin. IOS e viene illustrato come implementare i passaggi nel server.
...
Il Framework di social networking fornisce un'API unificata per l'interazione con i social network inclusi _Twitter_ e _Facebook_, così come _SinaWeibo_ per gli utenti in Cina.
