...
Infine, l'app deve usare un file di storyboard per presentare la schermata di avvio, anziché usare un set di file di immagine con **estensione png** e supportare tutti e quattro gli orientamenti dell'interfaccia (verticale, verticale verticale, orizzontale sinistro e orizzontale a destra) da considerare per l'esecuzione in una diapositiva sopra il pannello o in modalità di visualizzazione divisa.
...
In primo luogo, viene eseguito l'override della `CanBecomeFirstResponder` proprietà e viene restituito in `true` modo che il controller di visualizzazione o visualizzazione possa ricevere input da tastiera. 

Successivamente, si esegue l'override della `KeyCommands` proprietà e si crea un nuovo oggetto `UIKeyCommand` per la sequenza di tasti **Command-N** . Quando la sequenza di tasti è attivata, viene chiamato il `NewEntry` Metodo (che viene esposto a iOS 9 usando il `Export` comando) per eseguire l'azione richiesta.
...
- Se si usa il `AVPlayerLayer` per visualizzare il video, non creare un'istanza di `AVPictureInPictureController` .
