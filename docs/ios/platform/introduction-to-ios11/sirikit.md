...
Per l'interfaccia, ad esempio `IINCreateTaskListIntent` , il metodo richiesto è `HandleCreateTaskList` . Sono disponibili quattro metodi facoltativi che forniscono un maggiore controllo sull'interazione Siri:

- `ResolveTitle`: Convalida il titolo, imposta un titolo predefinito (se appropriato) o segnala che i dati non sono necessari.
- `ResolveTaskTitles`: Consente di convalidare l'elenco delle attività pronunciate dall'utente.
- `ResolveGroupName`: Convalida il nome del gruppo, sceglie un gruppo predefinito o segnala che i dati non sono necessari.
- `ConfirmCreateTaskList`: Verifica che il codice sia in grado di eseguire l'operazione richiesta, ma non lo esegue (solo i `Handle*` metodi devono modificare i dati).
...
A ogni metodo è stato passato un tipo di finalità specifico, che contiene tutte le informazioni di Siri analizzate dalla richiesta dell'utente e possibilmente aggiornate nei `Resolve*` metodi e `Confirm*` .
...
I metodi obbligatori `Handle*` e facoltativi `Confirm*` indicano un codice di risposta impostando un valore sull'oggetto che passano al gestore di completamento. Le risposte provengono dall' `INCreateTaskListIntentResponseCode` enumerazione:
...
3. Chiamare il `INPreferences.RequestSiriAuthorization` metodo nell'app per richiedere all'utente di consentire le interazioni Siri.
...
La classe deve ereditare da `INExtension` e, poiché l'esempio sta per gestire gli elenchi e le note Intent, implementa anche `IINNotebookDomainHandling` .

> [!NOTE]
>
> - In .NET esiste una convenzione che prevede che le interfacce siano precedute da un capitale `I` , a cui Novell aderisce quando si associano protocolli da iOS SDK.
...
> - Per il `Intents` Framework, i tipi sono preceduti da `IN*` (ad esempio, `INExtension`) ma queste _non_ sono interfacce.
> - Segue anche che i protocolli (che diventano interfacce in C#) si concludono con due `I` , ad esempio `IINAddTasksIntentHandling` .
...
1. **Elaborare lo scopo** : i dati analizzati da Siri vengono resi disponibili in un `intent` oggetto specifico del tipo di finalità. È possibile che l'app abbia convalidato i dati usando metodi facoltativi `Resolve*` .
...
3. **Fornire la risposta** : usare il `completion` gestore per inviare una risposta a Siri per leggere/visualizzare l'utente:
...
Si noti che `null` viene passato come secondo parametro alla risposta. si tratta del parametro dell'attività utente e, quando non viene specificato, verrà utilizzato un valore predefinito.
È possibile impostare un tipo di attività personalizzato purché l'app iOS la supporti tramite la `NSUserActivityTypes` chiave in **info. plist**. È quindi possibile gestire questo caso quando l'app viene aperta ed eseguire operazioni specifiche, ad esempio l'apertura di un controller di visualizzazione pertinente e il caricamento dei dati dall'operazione Siri.

Nell'esempio viene inoltre hardcoded il `Success` risultato, ma in scenari reali è necessario aggiungere una segnalazione errori corretta.
...
- Il file **titles. plist** viene configurato nelle **opzioni del progetto > compilare > la firma del bundle iOS**.
  ...
  [![Opzioni progetto che mostrano i diritti impostati correttamente](sirikit-images/set-entitlements-sml.png)](sirikit-images/set-entitlements.png#lightbox)
