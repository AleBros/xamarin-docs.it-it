...
Il Framework di social networking include un controller denominato `SLComposeViewController` che presenta una vista fornita dal sistema per la modifica e l'invio di un tweet. La schermata seguente mostra un esempio di questa visualizzazione:
...
Per usare un `SLComposeViewController` con Twitter, è necessario creare un'istanza del controller chiamando il metodo `FromService` con `SLServiceType.Twitter` come illustrato di seguito:

```csharp
var slComposer = SLComposeViewController.FromService (SLServiceType.Twitter);
```

Una volta restituita l'istanza di `SLComposeViewController`, può essere usata per presentare un'interfaccia utente per pubblicare post su Twitter. Tuttavia, la prima cosa da fare è verificare la disponibilità del social network, in questo caso Twitter, chiamando `IsAvailable`:
...
Una volta inizializzato, chiamando `PresentVIewController` viene visualizzata la vista creata dall'`SLComposeViewController`. L'utente può quindi modificare e inviare il tweet, oppure annullarne l'invio. In entrambi i casi, il controller deve essere chiuso nel `CompletionHandler`, in cui è anche possibile controllare il risultato per verificare se il tweet è stato inviato o annullato, come mostrato di seguito:
...
Il codice seguente illustra l'uso del `SLComposeViewController` per presentare una visualizzazione usata per inviare un tweet:
...
Il Framework sociale include anche il supporto per l'esecuzione di richieste HTTP ai social network. Incapsula la richiesta in una classe `SLRequest` usata per la destinazione dell'API del social network specifico.
...
Ad esempio, il `SLComposeViewController` può essere usato esattamente come nell'esempio di Twitter illustrato in precedenza, l'unico diverso è passare alle impostazioni e alle opzioni specifiche di Facebook. Esempio:
...
Quando viene usato con Facebook, il `SLComposeViewController` Visualizza una visualizzazione che sembra quasi identica all'esempio di Twitter, mostrando **Facebook** come titolo in questo caso:
