...
L'aggiunta di una mappa a un'applicazione viene eseguita aggiungendo un' `MKMapView` istanza alla gerarchia di visualizzazione, come mostrato di seguito:
...
`MKMapView`supporta 3 stili diversi di Maps. Per applicare uno stile mappa, è sufficiente impostare la `MapType` proprietà su un valore dell' `MKMapType` enumerazione:
...
Queste funzionalità possono essere abilitate o disabilitate semplicemente impostando le `ZoomEnabled` `ScrollEnabled` proprietà e dell' `MKMapView` istanza, in cui il valore predefinito è true per entrambi. Per visualizzare, ad esempio, una mappa statica, è sufficiente impostare le proprietà appropriate su false:
...
## <a name="user-location"></a>Ubicazione degli utenti
...
Si noti che nelle versioni di iOS precedenti alla 8,0, il tentativo di chiamare comporterà `RequestWhenInUseAuthorization` un errore. Assicurarsi di controllare la versione di iOS prima di effettuare la chiamata se si intende supportare versioni precedenti a 8.
...
Dopo aver aggiornato **info. plist** e aver richiesto all'utente l'autorizzazione per accedere alla propria posizione, è possibile visualizzare la posizione dell'utente sulla mappa impostando la `ShowsUserLocation` proprietà su true:
...
Map Kit usa il modello di delega iOS per aggiungere annotazioni a una mappa, in cui la `Delegate` proprietà di `MKMapView` è impostata su un'istanza di un oggetto `MKMapViewDelegate` . Si tratta dell'implementazione di questo delegato responsabile della restituzione dell'oggetto `MKAnnotationView` per un'annotazione.

Per aggiungere un'annotazione, prima viene aggiunta l'annotazione chiamando `AddAnnotations` sull' `MKMapView` istanza:
...
Quando la posizione dell'annotazione diventa visibile sulla mappa, il chiamerà il `MKMapView` metodo del delegato `GetViewForAnnotation` per ottenere l'oggetto `MKAnnotationView` da visualizzare.

Il codice seguente, ad esempio, restituisce un oggetto fornito dal sistema `MKPinAnnotationView` :
...
Per conservare la memoria, `MKMapView` consente di raggruppare la visualizzazione dell'annotazione per il riutilizzo, in modo analogo al modo in cui vengono riutilizzate le celle della tabella. Il recupero di una visualizzazione annotazione dal pool viene effettuato con una chiamata a `DequeueReusableAnnotation` :
...
Come indicato in precedenza, un'annotazione può facoltativamente visualizzare un callout. Per visualizzare un callout semplicemente impostato `CanShowCallout` su true in `MKAnnotationView` . Ciò comporta la visualizzazione del titolo dell'annotazione quando l'annotazione viene toccata, come illustrato:
...
Per gestire l'utente toccando l'accessorio corretto, è sufficiente implementare il `CalloutAccessoryControlTapped` metodo in `MKMapViewDelegate` :
...
- Creazione di una vista per la sovrimpressione in `MKMapViewDelegate` .

Il modello per la sovrimpressione può essere qualsiasi `MKShape` sottoclasse. Novell. iOS include `MKShape` sottoclassi per poligoni, polilinee e cerchi, rispettivamente tramite le `MKPolygon` classi, `MKPolyline` e `MKCircle` .

Il codice seguente, ad esempio, viene usato per aggiungere un oggetto `MKCircle` :
...
La visualizzazione di una sovrapposizione è un' `MKOverlayView` istanza restituita da `GetViewForOverlay` nell'oggetto `MKMapViewDelegate` . Ogni `MKShape` ha un oggetto corrispondente `MKOverlayView` che sa come visualizzare la forma specificata. Per la `MKPolygon` presenza di `MKPolygonView` . Analogamente, `MKPolyline` corrisponde a `MKPolylineView` e a per la presenza di `MKCircle` `MKCircleView` .

Il codice seguente, ad esempio, restituisce un oggetto `MKCircleView` per un oggetto `MKCircle` :
...
1. Creare un `MKLocalSearch` oggetto da `MKLocalSearchRequest` .
1. Chiamare il `Start` metodo sull' `MKLocalSearch` oggetto.
...
Un modo per accettare l'input di ricerca è con un oggetto `UISearchBar` , fornito da un oggetto e visualizzerà `UISearchController` i risultati in una tabella.
...
In questo frammento di codice è stato creato un altro controller di visualizzazione personalizzato, `searchResultsController` che Visualizza i risultati della ricerca e quindi questo oggetto è stato usato per creare l'oggetto controller di ricerca. È stato anche creato un nuovo aggiornamento di ricerca, che diventa attivo quando l'utente interagisce con la barra di ricerca. Riceve notifiche sulle ricerche con ogni pressione di tasto ed è responsabile dell'aggiornamento dell'interfaccia utente.
Si esaminerà come implementare il `searchResultsController` e il `searchResultsUpdater` più avanti in questa guida.
...
Per visualizzare i risultati della ricerca, è necessario creare un controller di visualizzazione personalizzato. in genere `UITableViewController` . Come illustrato sopra, `searchResultsController` viene passato al costruttore di `searchController` quando viene creato.
...
`SearchResultsUpdater`Funge da Mediator tra la `searchController` barra di ricerca e i risultati della ricerca.

In questo esempio è necessario creare innanzitutto il metodo di ricerca in `SearchResultsViewController` . A tale scopo, è necessario creare un `MKLocalSearch` oggetto e usarlo per eseguire una ricerca di un oggetto `MKLocalSearchRequest` , i risultati vengono recuperati in un callback passato al `Start` metodo dell' `MKLocalSearch` oggetto. I risultati vengono quindi restituiti in un `MKLocalSearchResponse` oggetto contenente una matrice di `MKMapItem` oggetti:
...
Quindi, in questa `MapViewController` sezione verrà creata un'implementazione personalizzata di `UISearchResultsUpdating` , che viene assegnata alla `SearchResultsUpdater` proprietà del `searchController` nella sezione [aggiunta di un'interfaccia utente di ricerca locale](#Adding_a_Local_Search_UI) :
