...
Per ulteriori informazioni sull'utilizzo di `UIWebView`, vedere le seguenti ricette:
...
È importante notare che `WKWebView` è nello spazio dei nomi WebKit, quindi è necessario aggiungere questa direttiva using all'inizio della classe.

`WKWebView` può essere usato anche nelle app Novell. Mac e, pertanto, è consigliabile usarlo se si sta creando un'app Mac/iOS multipiattaforma.
