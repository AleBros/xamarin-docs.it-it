...
- È possibile `Clean` , `Rebuild` o eliminare manualmente le `bin` `obj` Directory e.
...
Prima di eliminare `bin` le `obj` Directory e, eseguirne il zip e salvarle per una diagnosi successiva, se necessario. Probabilmente è possibile semplicemente `Clean` il progetto di applicazione Novell. Android per ricominciare a funzionare.
...
È possibile generare informazioni aggiuntive su GC, ad esempio le informazioni di temporizzazione, impostando la `MONO_LOG_LEVEL` variabile di ambiente su `debug` :
...
Nel `GC_BRIDGE` messaggio `num-objects` è il numero di oggetti Bridge che questo passaggio sta prendendo in considerazione e `num_hash_entries` è il numero di oggetti elaborati durante la chiamata del codice Bridge.

Nei `GC_MINOR` messaggi e `GC_MAJOR` `total` è l'intervallo di tempo durante il quale il mondo è sospeso (nessun thread è in esecuzione), mentre `bridge` è la quantità di tempo impiegato nel codice di elaborazione del Bridge (che gestisce la VM Java). Il mondo *non* viene sospeso mentre viene eseguita l'elaborazione del Bridge.
...
È possibile eseguire una query sui conteggi GREF e WREF eseguendo una query sull' `JniRuntime` oggetto.
...
Questo errore si verifica quando l'emulatore non viene avviato dall'interno di Visual Studio. Quando si avvia l'emulatore al di fuori di Visual Studio, è necessario passare le `-partition-size 512` Opzioni, ad esempio
...
## <a name="install_failed_invalid_apk-when-installing-a-package"></a>INSTALLAZIONE dell' \_ apk non valida non riuscita \_ \_ durante l'installazione di un pacchetto
...
- All'interno di Visual Studio:
  ...
  - Aggiornare il campo del nome del pacchetto.
    - Se viene visualizzato il messaggio &ldquo; non è stato trovato file AndroidManifest. XML. Fare clic per aggiungerne uno. &rdquo; fare clic sul collegamento e quindi aggiornare il campo del nome del pacchetto.
...
## <a name="install_failed_missing_shared_library-when-installing-a-package"></a>INSTALLAZIONE \_ \_ libreria condivisa mancante non riuscita \_ \_ durante l'installazione di un pacchetto
...
Il pacchetto Android specifica quali librerie condivise sono necessarie con l' `<uses-library/>` elemento. Se una libreria *richiesta* non è presente nel dispositivo di destinazione (ad esempio `//uses-library/@android:required` , è *true*, che è l'impostazione predefinita), l'installazione del pacchetto avrà esito negativo con l' *installazione non \_ riuscita \_ \_ \_ libreria condivisa mancante*.

Per determinare quali librerie condivise sono necessarie, visualizzare il *generated* 
 file**file AndroidManifest. XML** generato (ad esempio, **obj \\ debug \\ Android \\ file AndroidManifest. XML**) e cercare gli `<uses-library/>` elementi. `<uses-library/>`gli elementi possono essere aggiunti manualmente nel file ** \\ file AndroidManifest. XML delle proprietà** del progetto e tramite l' [attributo personalizzato UsesLibraryAttribute](xref:Android.App.UsesLibraryAttribute).

Ad esempio, se si aggiunge un riferimento a un assembly *mono. Android. GoogleMaps. dll* , si aggiunge in modo implicito un oggetto `<uses-library/>` per la libreria condivisa di Google Maps.

## <a name="install_failed_update_incompatible-when-installing-a-package"></a>INSTALLAZIONE dell' \_ aggiornamento non riuscito non \_ \_ compatibile durante l'installazione di un pacchetto
...
Quando si verifica questa situazione, l'installazione del pacchetto avrà esito negativo con un errore di installazione \_ \_ dell'aggiornamento non \_ compatibile, perché il nome del pacchetto non è stato modificato durante la chiave di firma. Il [log di debug Android](~/android/deploy-test/debugging/android-debug-log.md) conterrà anche un messaggio simile a:
...
## <a name="install_failed_uid_changed-when-installing-a-package"></a>INSTALLAZIONE \_ \_ di UID non riuscita \_ modificata durante l'installazione di un pacchetto

Quando viene installato un pacchetto Android, viene assegnato un *ID utente* (UID).
In *alcuni casi*, per motivi sconosciuti, quando si installa su un'app già installata, l'installazione avrà esito negativo con `INSTALL_FAILED_UID_CHANGED` :
...
## <a name="the-outpath-property-is-not-set-for-project-ldquomyappcsprojrdquo"></a>La proprietà outPath non è impostata per il progetto &ldquo; MyApp. csproj&rdquo;

Ciò significa in genere che si dispone di un computer HP e che la piattaforma della variabile &ldquo; &rdquo; di ambiente è stata impostata su un valore simile a MCD o Honolulu. Questa operazione è in conflitto con la proprietà della piattaforma MSBuild che in genere è impostata su &ldquo; qualsiasi CPU &rdquo; o &ldquo; x86 &rdquo; . È necessario rimuovere questa variabile di ambiente dal computer prima che MSBuild possa funzionare:
...
Il problema è che Novell. Android esegue erroneamente il marshalling dei tipi generici annidati. `List<IDictionary<string, object>>`Viene eseguito il marshalling in un oggetto [java. lang. ArrrayList](xref:Java.Util.ArrayList), ma l'oggetto `ArrayList` contiene le `mono.android.runtime.JavaObject` istanze (che fanno riferimento alle `Dictionary<string, object>` istanze) anziché un elemento che implementa [java. util. map](xref:Java.Util.IMap), causando la seguente eccezione:
...
La soluzione alternativa consiste nell'usare i [tipi di raccolta Java](~/android/internals/api-design.md) forniti anziché i `System.Collections.Generic` tipi per &ldquo; i &rdquo; tipi interni. Questa operazione determinerà tipi Java appropriati per il marshalling delle istanze. Il codice seguente è più complicato del necessario per ridurre le durate Gref. Può essere semplificato per modificare il codice originale tramite `s/List/JavaList/g` e `s/Dictionary/JavaDictionary/g` se la durata Gref non è un problema.
...
Occasionalmente, il [log di debug Android](~/android/deploy-test/debugging/android-debug-log.md) menzionerà le eccezioni NullReferenceException che &ldquo; non possono verificarsi &rdquo; o provengono dal codice di runtime di mono per Android poco prima che l'app muoia:
...
Questo problema può verificarsi quando il runtime di Android decide di interrompere il processo, operazione che può verificarsi per diversi motivi, tra cui il raggiungimento del limite GREF della destinazione o l'esecuzione di un &ldquo; errore &rdquo; in JNI.
...
Per usare \# la dinamica C nell'applicazione o nella libreria, è necessario aggiungere System. Core. dll, Microsoft. CSharp. dll e mono. CSharp. dll al progetto.
