...
description: Questo articolo spiega come usare le funzionalità di accelerazione hardware del computer per ottimizzare le prestazioni dell'emulatore Android di Google.
ms.prod: xamarin
...
ms.locfileid: "34806816"
---
# <a name="hardware-acceleration-for-emulator-performance"></a>Accelerazione hardware per le prestazioni dell'emulatore

_Questo articolo spiega come usare le funzionalità di accelerazione hardware del computer per ottimizzare le prestazioni dell'emulatore Android di Google._

## <a name="overview"></a>Panoramica

Per gli sviluppatori, Visual Studio semplifica il test e il debug del codice delle applicazioni Xamarin.Android con l'emulatore Android di Google nelle situazioni in cui un dispositivo Android non è disponibile o risulta poco pratico.
...
L'emulatore Android di Google usa automaticamente l'accelerazione hardware se vengono soddisfatti i criteri seguenti:
...
Per informazioni sull'avvio e il debug con l'emulatore Android, vedere [Debug con l'emulatore Android di Google](~/android/deploy-test/debugging/android-sdk-emulator/index.md).
...
L'uso di Microsoft Hyper-V è vivamente consigliato agli sviluppatori che usano l'Aggiornamento di Windows 10 (aprile 2018 o versione successiva) per accelerare l'emulatore Android di Google. Per usare l'emulatore Android di Google con Hyper-V:
...
3. **Installare [Visual Studio 15.8 Preview 1 o versione successiva](https://www.visualstudio.com/vs/preview/)**.
   Questa versione di Visual Studio fornisce supporto IDE per l'esecuzione dell'emulatore Android di Google con Hyper-V.
...
Se si sviluppa in un computer con una CPU Intel che dispone di funzionalità VT, è possibile trarre vantaggio da HAXM per velocizzare notevolmente l'emulatore Android di Google (se non si è certi che la CPU supporti VT, vedere [Il mio processore supporta Intel Virtualization Technology?](https://www.intel.com/content/www/us/en/support/processors/000005486.html)).
...
Prima di usare l'emulatore Android di Google per la prima volta, è consigliabile verificare che HAXM sia installato e disponibile per l'uso con l'emulatore Android di Google.
