...
- ***Layout*** &ndash; di base formato di presentazione compatto e fisso.
...
[![Esempio di notifica della schermata di blocco](local-notifications-images/05-lockscreen-notification-sml.png)](local-notifications-images/05-lockscreen-notification.png#lightbox)
...
- **Priorità** &ndash; di Il livello di priorità determina come e quando vengono presentate le notifiche. Ad esempio, in Android 5,0, le notifiche ad alta priorità vengono visualizzate come notifiche Heads-up.
...
- **Categoria** &ndash; di Informa il sistema come gestire la notifica in diverse circostanze, ad esempio quando il dispositivo è in modalità *non disturbare* .
...
- ***Immagine*** &ndash; di In modalità a contratti, Visualizza solo il testo del messaggio. In modalità espansa Visualizza il testo e un'immagine.
...
[![Schermata Scarica notifiche per l'app YouTube](local-notifications-images/28-yt-download-sml.png)](local-notifications-images/28-yt-download.png#lightbox)
...
[![Schermata generale delle notifiche per l'app YouTube](local-notifications-images/29-yt-general-sml.png)](local-notifications-images/29-yt-general.png#lightbox)
...
`NotificationCompat.Builder`viene inoltre illustrato.
...
2. Chiamare diversi metodi sull' `NotificationCompat.Builder` oggetto per impostare le opzioni di notifica.
...
Nell'esempio di codice riportato di seguito viene illustrato come utilizzare `NotificationCompat.Builder` per generare una notifica di base. Si noti che i `NotificationCompat.Builder` metodi supportano il [concatenamento](https://en.wikipedia.org/wiki/Method_chaining)dei metodi, ovvero ogni metodo restituisce l'oggetto generatore, quindi è possibile usare il risultato dell'ultima chiamata al metodo per richiamare la chiamata al metodo successiva:
...
In questo esempio viene creata un'istanza di un nuovo `NotificationCompat.Builder` oggetto denominato `builder` , insieme all'ID del canale di notifica da usare. Il titolo e il testo della notifica vengono impostati e l'icona di notifica viene caricata da **Resources/assortable/ic_notification. png**. La chiamata al metodo del generatore di notifiche `Build` Crea un oggetto notifica con queste impostazioni. Il passaggio successivo consiste nel chiamare il `Notify` metodo di Notification Manager. Per individuare Notification Manager, chiamare `GetSystemService` , come illustrato in precedenza.
...
Se si vuole che la notifica riproduca anche un suono, è possibile chiamare il metodo [Sedefaults](xref:Android.App.Notification.Builder.SetDefaults*) del generatore di notifiche e passare il `NotificationDefaults.Sound` flag:
...
Questa chiamata a `SetDefaults` comporterà la riproduzione di un suono da parte del dispositivo quando viene pubblicata la notifica. Se si vuole che il dispositivo vibra anziché riprodurre un suono, è possibile passare `NotificationDefaults.Vibrate` a `SetDefaults.` se si vuole che il dispositivo riproduca un suono e vibrare il dispositivo, è possibile passare entrambi i flag a `SetDefaults` :

```csharp
builder.SetDefaults (NotificationDefaults.Sound | NotificationDefaults.Vibrate);
```

Se si Abilita il suono senza specificare un suono, Android usa il suono di notifica di sistema predefinito. Tuttavia, è possibile modificare il suono che verrà riprodotto chiamando il metodo [sesound](xref:Android.App.Notification.Builder.SetSound*) del generatore di notifiche. Ad esempio, per riprodurre il suono di allarme con la notifica, anziché il suono di notifica predefinito, è possibile ottenere l'URI per il suono di allarme dal [RingtoneManager](xref:Android.Media.RingtoneManager) e passarlo a `SetSound` :
...
Dopo aver creato un oggetto notifica, è possibile impostare le proprietà delle notifiche nell'oggetto notifica, anziché configurarle in anticipo tramite `NotificationCompat.Builder` metodi. Ad esempio, invece di chiamare il `SetDefaults` metodo per abilitare la vibrazione per una notifica, è possibile modificare direttamente il flag di bit della proprietà [valori predefiniti](xref:Android.App.Notification.Defaults) della notifica:
...
Se si desidera aggiornare il contenuto di una notifica dopo che è stata pubblicata, è possibile riutilizzare l' `NotificationCompat.Builder` oggetto esistente per creare un nuovo oggetto notifica e pubblicare la notifica con l'identificatore dell'ultima notifica. Ad esempio:
...
In questo esempio, l' `NotificationCompat.Builder` oggetto esistente viene utilizzato per creare un nuovo oggetto notifica con un titolo e un messaggio diversi.
...
Il corpo della notifica precedente viene riutilizzato &ndash; solo il titolo e il testo della notifica viene modificato mentre la notifica viene visualizzata nel cassetto delle notifiche. Il testo del titolo passa da "notifica di esempio" a "notifica aggiornata" e il testo del messaggio cambia da "Hello World! Questa è la mia prima notifica! " a "modificato in questo messaggio".
...
Questo codice è molto simile al codice di notifica nella sezione precedente, ad eccezione del fatto che un `PendingIntent` oggetto viene aggiunto all'oggetto notifica. In questo esempio, l'oggetto `PendingIntent` è associato all'attività dell'app di origine prima che venga passata al metodo [SetContentIntent](xref:Android.App.Notification.Builder.SetContentIntent*) del generatore di notifiche. Il `PendingIntentFlags.OneShot` flag viene passato al `PendingIntent.GetActivity` metodo in modo che l'oggetto `PendingIntent` venga usato una sola volta. Quando viene eseguito questo codice, viene visualizzata la seguente notifica:
...
Nella maggior parte dei casi, la navigazione all'indietro dall'attività di notifica deve restituire l'utente dall'app e tornare alla schermata iniziale. Per gestire lo stack indietro, l'app usa la classe [TaskStackBuilder](xref:Android.App.TaskStackBuilder) per creare un oggetto `PendingIntent` con uno stack indietro.

Un'altra considerazione reale è che l'attività di origine potrebbe dover inviare dati all'attività di notifica. Ad esempio, la notifica può indicare che è arrivato un messaggio di testo e l'attività di notifica (una schermata di visualizzazione dei messaggi) richiede l'ID del messaggio per visualizzare il messaggio all'utente. L'attività che crea l'oggetto `PendingIntent` può utilizzare il metodo [Intent. PutExtra](xref:Android.Content.Intent.PutExtra*) per aggiungere dati, ad esempio una stringa, allo scopo, in modo che questi dati vengano passati all'attività di notifica.
...
In questo esempio di codice, l'applicazione è costituita da due attività: `MainActivity` (che contiene il codice di notifica precedente) e `SecondActivity` , la schermata visualizzata dall'utente dopo aver toccato la notifica. Quando viene eseguito questo codice, viene presentata una semplice notifica (simile all'esempio precedente). Se si tocca la notifica, l'utente viene `SecondActivity` visualizzato sullo schermo:

![Schermata seconda attività](local-notifications-images/11-second-activity.png)

Il messaggio stringa (passato nel metodo di Intent `PutExtra` ) viene recuperato in `SecondActivity` tramite questa riga di codice:
...
Questo messaggio recuperato, "Greetings from MainActivity!", viene visualizzato nella `SecondActivity` schermata, come illustrato nello screenshot precedente. Quando l'utente preme il pulsante **indietro** mentre è in `SecondActivity` , la navigazione esce dall'app e torna alla schermata precedente all'avvio dell'app.
...
Per impostazione predefinita, le notifiche sono un semplice formato di layout di base in Android, ma è possibile migliorare questo formato di base effettuando ulteriori `NotificationCompat.Builder` chiamate al metodo. In questa sezione verrà illustrato come aggiungere un'icona foto di grandi dimensioni alla notifica. verranno visualizzati esempi di come creare notifiche di layout espanse.
...
Di seguito è riportato un esempio di una notifica di base di Android 5,0 &ndash; che visualizza solo l'icona dell'app piccola:

![Notifica normale di esempio](local-notifications-images/13-sample-notification.png)

Di seguito è riportata una schermata della notifica dopo averla modificata per visualizzare un'icona grande &ndash; che usa un'icona creata da un'immagine di una scimmia di codice Novell:
...
Per creare una notifica di *testo di grandi dimensioni* , creare un'istanza di un `NotificationCompat.Builder` oggetto, come in precedenza, quindi creare un'istanza e aggiungere un oggetto [BigTextStyle](xref:Android.App.Notification.BigTextStyle) all' `NotificationCompat.Builder` oggetto. Ecco un esempio:
...
Si noti che quando la notifica viene visualizzata in formato Compact, viene visualizzato il testo della notifica, ovvero il testo passato al metodo del generatore di notifiche `SetContentText` , come illustrato in precedenza. Tuttavia, quando la notifica viene espansa per rivelare l'immagine, viene visualizzato il testo di riepilogo sopra l'immagine.

Per creare una notifica *immagine* , creare un'istanza di un `NotificationCompat.Builder` oggetto come in precedenza, quindi creare e inserire un oggetto [BigPictureStyle](xref:Android.App.Notification.BigPictureStyle) nell' `NotificationCompat.Builder` oggetto. Ad esempio:
...
Analogamente al `SetLargeIcon` metodo di `NotificationCompat.Builder` , il metodo [BigPicture](xref:Android.App.Notification.BigPictureStyle.BigPicture*) di `BigPictureStyle` richiede una bitmap dell'immagine che si desidera visualizzare nel corpo della notifica. In questo esempio, il metodo [DecodeResource](xref:Android.Graphics.BitmapFactory.DecodeResource*) di `BitmapFactory` legge il file di immagine che si trova in **Resources/assortable/x_bldg. png** e lo converte in una bitmap.
...
Per creare una notifica della *posta in arrivo* , creare un'istanza di un `NotificationCompat.Builder` oggetto, come in precedenza, e aggiungere un oggetto [InboxStyle](xref:Android.App.Notification.InboxStyle) a `NotificationCompat.Builder` . Ecco un esempio:
...
Per aggiungere nuove righe di testo al corpo della notifica, chiamare il metodo [AddLine](xref:Android.App.Notification.InboxStyle.AddLine*) dell' `InboxStyle` oggetto (l'altezza massima della notifica della *posta in arrivo* è 256 DP). Si noti che, a differenza dello stile di *testo di grandi dimensioni* , lo stile della *posta in arrivo* supporta le singole righe di testo nel corpo della notifica.

È anche possibile usare lo stile della *posta in arrivo* per tutte le notifiche che devono visualizzare singole righe di testo in un formato espanso. Ad esempio, è possibile usare lo stile di notifica della *posta in arrivo* per combinare più notifiche in sospeso in una notifica &ndash; di riepilogo. è possibile aggiornare una singola notifica dello stile della *posta in arrivo* con le nuove righe di contenuto della notifica (vedere [aggiornamento di una notifica](#updating-a-notification) precedente), anziché generare un flusso continuo di notifiche nuove, per lo più simili.
...
Per impostare la priorità di una notifica, chiamare il metodo [sepriority](xref:Android.App.Notification.Builder.SetPriority*) dell' `NotificationCompat.Builder` oggetto, passando il livello di priorità. Ad esempio:
...
- `NotificationVisibility.Private`&ndash;Solo le informazioni essenziali vengono visualizzate nella schermata di blocco sicura, ad esempio l'icona di notifica e il nome dell'app che l'ha inviata, ma i dettagli della notifica restano nascosti. Tutte le notifiche vengono predefinite a `NotificationVisibility.Private` .
...
Per impostare la visibilità di una notifica, le app chiamano il `SetVisibility` metodo dell' `NotificationCompat.Builder` oggetto, passando l'impostazione di visibilità. Ad esempio, questa chiamata a `SetVisibility` Crea la notifica `Private` :

```csharp
builder.SetVisibility (NotificationVisibility.Private);
```

Quando `Private` viene inviata una notifica, nella schermata di blocco protetto vengono visualizzati solo il nome e l'icona dell'app. Anziché il messaggio di notifica, l'utente Visualizza "sbloccare il dispositivo per visualizzare la notifica":
...
Quando l'utente configura non *disturbare* il blocco di tutti gli interrupt ad eccezione delle chiamate telefoniche (come illustrato nello screenshot precedente), Android consente di presentare notifiche con un'impostazione di categoria di `Notification.CategoryCall` da presentare mentre il dispositivo è in modalità *non disturbare* . Si noti che le `Notification.CategoryAlarm` notifiche non vengono mai bloccate in modalità *non di disturbo* .
...
In modo analogo, l'app può usare `NotificationCompat.InboxStyle` `NotificationCompat.BigPictureStyle` rispettivamente e per la *posta in arrivo* e gli stili di *immagine* .

### <a name="notification-priority-and-category"></a>Priorità e categoria delle notifiche

`NotificationCompat.Builder`supporta il `SetPriority` Metodo (disponibile a partire da Android 4,1). Tuttavia, il `SetCategory` metodo *non* è supportato da `NotificationCompat.Builder` perché le categorie fanno parte del nuovo sistema di metadati delle notifiche introdotto in Android 5,0.

Per supportare le versioni precedenti di Android, dove `SetCategory` non è disponibile, il codice può controllare il livello API in fase di esecuzione per chiamare in modo condizionale `SetCategory` quando il livello API è uguale o maggiore di Android 5,0 (livello API 21):
...
Poiché Android non supporta le notifiche della schermata di blocco prima di Android 5,0 (API level 21), non `NotificationCompat.Builder` supporta il `SetVisibility` metodo. Come spiegato in precedenza per `SetCategory` , il codice può controllare il livello API in fase di esecuzione e chiamare `SetVisiblity` solo quando è disponibile:
