...
Il generatore di binding utilizzerà il file **API. XML** come linee guida per la generazione delle C# classi wrapper necessarie. Il contenuto di questo file XML è una variante del formato di _progetto open source Android_ di Google.
...
Il generatore di associazioni creerà il codice seguente C# per la classe wrapper:
