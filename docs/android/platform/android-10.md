...
Se si usa un dispositivo fisico, ad esempio un pixel, è possibile scaricare l'aggiornamento di Android 10 selezionando il `System`  >   >  `System update` `Check for update` nelle impostazioni del telefono. Se si preferisce eseguire il flashing del dispositivo, vedere le istruzioni su come lampeggiare un' [immagine di fabbrica](https://developers.google.com/android/ota) o un' [immagine OTA](https://developers.google.com/android/ota) nel dispositivo.
...
- **Tema scuro:**  garantire un'esperienza coerente per gli utenti che abilitano il tema scuro a livello di sistema aggiungendo un [tema](https://developer.android.com/preview/features/darktheme) scuro o abilitando la [forza scura](https://developer.android.com/preview/features/darktheme#force_dark).

![Tema scuro](~/android/platform/android-10-images/darktheme.png)

- **Supporta la [navigazione](https://developer.android.com/preview/features/gesturalnav)**  gestuale nell'app tramite Edge-to-Edge e assicurandosi che i movimenti personalizzati siano complementari ai movimenti di navigazione del sistema.

![Spostamento movimenti](~/android/platform/android-10-images/gesturenavigation.png)

- **Ottimizza per i ripieghevoli:**  offri esperienze Edge-to-Edge senza problemi sui dispositivi innovativi attuali [ottimizzando i ripieghevoli](https://developer.android.com/preview/features/foldables).
...
- **Biometria migliore:**  se si usa l'autenticazione biometrica, passare a [BiometricPrompt](https://developer.android.com/reference/androidx/biometric/BiometricPrompt), il modo migliore per supportare l'autenticazione con impronta digitale sui dispositivi moderni.
...
- **Codec migliori:**  per le app multimediali, provare [AV1](https://en.wikipedia.org/wiki/AV1) per streaming video e [HDR10 +](https://en.wikipedia.org/wiki/High-dynamic-range_video#HDR10+) per un video con intervallo dinamico elevato. Per lo streaming vocale e musicale, è possibile usare la codifica [Opus](http://opus-codec.org/) e per i musicisti è disponibile un'  [API MIDI nativa](https://developer.android.com/preview/features/midi).
- **API di rete migliori:**  se l'app gestisce i dispositivi di Internet delle cose tramite Wi-Fi, provare le nuove [API](https://developer.android.com/preview/features#peer2peer) di connessione di rete per funzioni quali la configurazione, il download o la stampa.
...
- Le [autorizzazioni per gli Intent a schermo `USE_FULL_SCREEN_INTENT`intero devono richiedere ](https://developer.android.com/about/versions/10/behavior-changes-10#full-screen-intents).
