...
2. **Nessuna app esistente è associata all'URI, ma è installata** &ndash; un'app di supporto In questo scenario l'utente non ha specificato un'app esistente, quindi Android userà l'applicazione di supporto installata per gestire la richiesta.
3. **Nessuna app esistente è associata all'URI, ma sono installate** &ndash; molte app di supporto Poiché sono presenti più applicazioni che supportano l'URI, viene visualizzata la finestra di dialogo di risoluzione dell'ambiguità e l'utente deve selezionare l'app che gestirà l'URI.
...
- **`DataScheme`**&ndash;Il filtro preventivo deve dichiarare `http` e/o `https` . Questi sono gli unici due schemi validi.
- **`DataHost`** Si &ndash; tratta del dominio da cui derivano gli URI.
- **`DataPathPrefix`** Si &ndash; tratta di un percorso facoltativo per le risorse nel sito Web.
...
> [!NOTE]
> L' `android:autoVerify` attributo deve essere impostato dal filtro preventivo &ndash; altrimenti Android non eseguirà la verifica.
