...
- [Contenuto utente](#user_content) &ndash; Gestione file utente semplificata con il Framework di accesso all'archiviazione; la stampa di immagini, siti Web e altro contenuto è più semplice grazie alle API di stampa migliorate.

- [Hardware](#hardware) &ndash; Trasforma qualsiasi app in una scheda NFC con emulazione di scheda basata su host NFC; eseguire sensori a basso consumo con `SensorManager` .
...
Per indicare ad Android di riattivare un'app durante un intervallo di tempo specificato, chiamare `SetWindow` sul [`AlarmManager`](xref:Android.App.AlarmManager) , passando il tempo minimo e massimo, in millisecondi, che può trascorrere prima che l'app venga riattivata e l'operazione da eseguire al riattivazione.
...
- Se si sta ottenendo un percorso di file o directory accedendo a una proprietà o chiamando un metodo su `Environment` , ad esempio[`GetExternalStorageDirectory`](xref:Android.OS.Environment.ExternalStorageDirectory)
  o[`GetExternalStoragePublicDirectory`](xref:Android.OS.Environment.GetExternalStoragePublicDirectory*)
  , l'app richiede l' `READ_EXTERNAL_STORAGE` `WRITE_EXTERNAL_STORAGE` autorizzazione o.
...
[WebView](xref:Android.Webkit.WebView) ha ottenuto un rifacimento in KitKat. La modifica più importante è stata aggiunta alla sicurezza per il caricamento di contenuto in un `WebView` . Sebbene la maggior parte delle applicazioni destinate alle versioni precedenti dell'API funzioni come previsto, è consigliabile testare le applicazioni che usano la `WebView` classe. Per ulteriori informazioni sulle API WebView interessate, vedere la documentazione relativa alla [migrazione a WebView in android 4,4 in Android](https://developer.android.com/guide/webapps/migrating.html) .
...
L'elemento statico dell'interfaccia utente che ospita il contenuto dinamico è un oggetto denominato base del *contenitore* o della *scena*. Nell'esempio seguente viene usato il Android Designer per creare un oggetto `RelativeLayout` denominato `container` :
...
Nell'esempio precedente viene usato `merge` per rendere il codice di visualizzazione più breve e semplificare la gerarchia di visualizzazione. Per altre informazioni sui `merge` layout, vedere [qui](https://android-developers.blogspot.com/2009/03/android-layout-tricks-3-optimize-by.html).

Una scena viene creata chiamando [`Scene.GetSceneForLayout`](xref:Android.Transitions.Scene.GetSceneForLayout*) , passando l'oggetto contenitore, l'ID risorsa del file di layout della scena e l'oggetto corrente `Context` , come illustrato nell'esempio di codice seguente:
...
> [!NOTE]
> È presente un [bug noto](https://code.google.com/p/android/issues/detail?id=62450) nella libreria delle transizioni Android che fa sì che le scene create con `GetSceneForLayout` si interrompano quando un utente passa attraverso un'attività la seconda volta. Una soluzione alternativa Java è descritta [qui](http://www.doubleencore.com/2013/11/new-transitions-framework/).
...
- `fitsSystemWindows`-Se si imposta la barra superiore o inferiore su transcluent, per impostazione predefinita il contenuto viene spostato sotto gli elementi trasparenti dell'interfaccia utente. L'impostazione di questa proprietà su `true` è un modo semplice per impedire la sovrapposizione di contenuto con gli elementi dell'interfaccia utente di sistema traslucidi.
...
Questa modifica richiede due azioni sul lato sviluppatore: prima di tutto, le app che richiedono contenuto dai provider devono essere aggiornate a un nuovo modo di richiedere il contenuto. In secondo luogo, è necessario modificare le applicazioni che scrivono i dati in `ContentProvider` modo da usare il nuovo Framework. Entrambi gli scenari dipendono dalla nuova[`DocumentsProvider`](xref:Android.Provider.DocumentsProvider)
.

#### <a name="documentsprovider"></a>DocumentsProvider

In KitKat le interazioni con `ContentProviders` sono astratte dalla `DocumentsProvider` classe. Questo significa che SAF non è interessato dalla posizione dei dati fisicamente, purché sia accessibile tramite l' `DocumentsProvider` API. I provider locali, i servizi cloud e i dispositivi di archiviazione esterni usano tutte la stessa interfaccia e vengono gestiti allo stesso modo, fornendo all'utente e allo sviluppatore un'unica posizione per interagire con il contenuto dell'utente.
...
È possibile indicare a KitKat che si vuole selezionare il contenuto usando l'interfaccia utente di SAF con lo `ActionOpenDocument` scopo, che significa che ci si vuole connettere a tutti i provider di contenuti disponibili per il dispositivo. È possibile aggiungere alcuni filtri a questa finalità specificando `CategoryOpenable` , il che significa che verrà restituito solo il contenuto che può essere aperto, ad esempio contenuto accessibile e utilizzabile. KitKat consente anche di filtrare il contenuto con `MimeType` . Ad esempio, il codice seguente Filtra i risultati dell'immagine specificando l'immagine `MimeType` :
...
`StartActivityForResult`La chiamata a avvia l'interfaccia utente di SAF, che l'utente può quindi individuare per scegliere un'immagine:
...
Dopo che l'utente ha scelto un'immagine, `OnActivityResult` restituisce l'oggetto `Android.Net.Uri` del file scelto. Nell'esempio di codice riportato di seguito viene visualizzata la selezione dell'immagine dell'utente:
...
Oltre a caricare contenuto dall'interfaccia utente di SAF, KitKat consente anche di salvare il contenuto in qualsiasi `ContentProvider` che implementi l' `DocumentProvider` API. Il salvataggio del contenuto usa un `Intent` con `ActionCreateDocument` :
...
Restituisce un oggetto `System.IO.Stream` , quindi l'intero processo di streaming può essere scritto in .NET.
...
KitKat crea automaticamente un oggetto [`PrintDocumentAdapter`](xref:Android.Print.PrintDocumentAdapter) per una visualizzazione Web con `WebView.CreatePrintDocumentAdapter` . La stampa di contenuto Web è un lavoro coordinato tra un [`WebViewClient`](xref:Android.Webkit.WebViewClient) che attende il caricamento del contenuto HTML e consente all'attività di rendere disponibile l'opzione di stampa nel menu opzioni e l'attività, che attende che l'utente selezioni l'opzione di stampa e chiami `Print` su `PrintManager` . In questa sezione viene illustrata la configurazione di base necessaria per stampare contenuto HTML a schermo intero.
...
Il processo di consiste nel garantire che i `WebViewClient` dati in vengano `WebView` caricati completamente prima che l'opzione Print venga visualizzata nel menu, operazione eseguita con il `OnPageFinished` metodo. `OnPageFinished`resta in attesa del completamento del caricamento del contenuto Web e indica all'attività di ricreare il menu delle opzioni con `InvalidateOptionsMenu` :
...
`OnPageFinished`imposta anche il `dataLoaded` valore su `true` , in modo che sia `OnCreateOptionsMenu` possibile ricreare il menu con l'opzione di stampa sul posto.
...
`Print`La chiamata di carica l'interfaccia utente di stampa, che elenca le opzioni per il processo di stampa. L'interfaccia utente offre agli utenti la possibilità di stampare o salvare il contenuto HTML in un file PDF, come illustrato negli screenshot seguenti:
...
HCE richiede che sia la funzionalità HCE che l' `Nfc` autorizzazione siano registrate con l'applicazione `AndroidManifest.xml` :
...
`OnSensorChanged`viene chiamato se il conteggio dei passaggi viene aggiornato mentre l'applicazione è in primo piano. Se l'applicazione viene attivata in background o il dispositivo è in stato `OnSensorChanged` di sospensione, non verrà chiamato. Tuttavia, i passaggi continueranno a essere conteggiati fino a quando non `UnregisterListener` viene chiamato il metodo.

Tenere presente che *il valore del numero di passaggi è cumulativo in tutte le applicazioni che registrano il sensore*. Ciò significa che anche se si disinstalla e si reinstalla l'applicazione e si inizializza la `count` variabile su 0 all'avvio dell'applicazione, il valore restituito dal sensore rimarrà il numero totale di passaggi eseguiti durante la registrazione del sensore, sia per l'applicazione che per un altro. È possibile impedire l'aggiunta dell'applicazione al contatore dei passaggi chiamando `UnregisterListener` su `SensorManager` , come illustrato nel codice seguente:
...
- *Sync ContentResolver at Custom Interval* : KitKat aggiunge una certa variabilità al momento in cui viene eseguita una richiesta di sincronizzazione. Sincronizzare un oggetto `ContentResolver` all'ora o all'intervallo personalizzato chiamando `ContentResolver.RequestSync` e passando un oggetto `SyncRequest` .

- *Distinzione tra controller* : in KitKat, ai controller vengono assegnati identificatori di tipo integer univoci a cui è possibile accedere tramite la proprietà del dispositivo `ControllerNumber` . In questo modo è più semplice distinguere tra i diversi giocatori in un gioco.
...
In questo articolo sono state introdotte alcune delle nuove API disponibili in Android 4,4 (API level 19) e sono state descritte le procedure consigliate per la transizione di un'applicazione a KitKat. Sono state descritte le modifiche apportate alle API che influiscono sull'esperienza utente, tra cui il *Framework di transizione* e le nuove opzioni per il *tema*. Successivamente, è stata introdotta la classe e il *Framework di accesso* alla risorsa di archiviazione `DocumentsProvider` , nonché le nuove *API di stampa*. Ha esplorato l' *emulazione delle schede basata su host NFC* e come usare i *sensori a basso consumo*, inclusi due nuovi sensori per tenere traccia dei passaggi dell'utente. Infine, è stato illustrato come acquisire demo in tempo reale delle applicazioni con la *registrazione dello schermo*e fornito un elenco dettagliato delle modifiche e delle aggiunte dell'API KitKat.
